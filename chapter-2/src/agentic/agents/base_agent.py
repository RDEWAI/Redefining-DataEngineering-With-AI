...
from src.agentic.library.repository import get_repository
from src.agentic.llm.base import LLMProvider, LLMResponse, Message
from src.agentic.llm.unified_client import UnifiedLLMClient

# Load .env from chapter-2 directory
# Path: base_agent.py -> agents/ -> agentic/ -> src/ -> chapter-2/
...
class BaseCodeExecutionAgent:
    ...
    def __init__(
        ...
        if db_path is None:
            db_path = os.getenv(
                "DB_PATH",
                str(Path(__file__).parent.parent.parent.parent / "data" / "duckdb" / "chapter2.db"),
            )
