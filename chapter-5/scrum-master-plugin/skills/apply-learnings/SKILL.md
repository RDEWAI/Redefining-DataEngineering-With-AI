...
```bash
cat memory/stories/learnings-queue.jsonl
```
...
```
Pending learnings:
- create-stories: 3 corrections
- validate-stories: 1 correction
```
...
```
Proposed learning for create-stories:
- **L-003** (2026-03-23): Always create separate stories for schema creation vs data loading — never combine DDL and DML in a single story.
...
1. Read the target skill file:
   `scrum-master-plugin/skills/{skill-name}/SKILL.md`
...
After all learnings are processed, update `memory/stories/learnings-queue.jsonl`:
