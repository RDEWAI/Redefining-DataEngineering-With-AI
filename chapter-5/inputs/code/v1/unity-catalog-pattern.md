...
- **One UC server per workspace** — `docker compose up -d server` runs
  `unitycatalog/unitycatalog:0.4.1` on port `8080`.
...
- Unity Catalog 0.4.1 REST API (`/api/2.1/unity-catalog/...`).
- `io.unitycatalog:unitycatalog-spark_2.13:0.4.1` jar (Maven coordinate
  — see `LIBRARIES.md`).
