...
class DqsRule:
    ...
    def action_if_failed(self) -> str:
        """Map DQS severity to SE action_if_failed."""
...
def _build_se_rule_entry(
    ...
    return {
        ...
        # source validation: true for query_dq (reconciliation checks both sides)
        "enable_for_source_dq_validation": is_query,
        "enable_for_target_dq_validation": True,
        "is_active": True,
        # alert on drops: true for statistical/reconciliation rules
        "enable_error_drop_alert": is_statistical,
        # integer percentage threshold
        "error_drop_threshold": int(rule.error_drop_threshold(default_threshold)),
        "query_dq_delimiter": "@",
...
def _canonicalize_table(table: str, layer_schemas: dict[str, str] | None = None) -> str:
    ...
    schema = _infer_schema_from_table(table, schemas)
