#!/usr/bin/env python3
"""PreToolUse hook: enforce read-only database queries from the BA agent.
...
def main():
    ...
    is_db_command = any(re.search(pattern, command, re.IGNORECASE) for pattern in DB_INDICATORS)
    ...
    for pattern in WRITE_PATTERNS:
        ...
        if match:
            _deny(
                ...
                f"The BA agent must only run read-only SELECT queries. "
