...
Implement `src/patient_360/gold/build_clinical_history.py` (or `build_patient_summary.py`) per LLD §5.3. Reads current-version `clinical_patients` (`is_current=TRUE`), joins to relevant facts, denormalizes via ARRAY<STRUCT> (LLD §5.3 / NFR-1), writes full overwrite to `warehouse/{env}/gold/patient_clinical_history/`. Inline SE via `dq_rules/patient_clinical_history.yml`. Empty-input: `fail` (LLD §5.3 — consumer table must have data).
...
- [ ] Output written with full overwrite to `warehouse/{env}/gold/patient_clinical_history/` [LLD §3.3]
...
AC2:
  - grep: {glob: "patient_360/src/patient_360/gold/build_*.py", pattern: "patient_clinical_history"}
AC3:
