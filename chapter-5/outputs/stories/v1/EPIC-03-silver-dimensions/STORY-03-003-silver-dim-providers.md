...
As a data engineer, I want transform `unity.bronze.synthea_providers` into Silver dimension `reference_providers` with SCD Type 2 so that the providers dimension preserves history and Gold tables get the latest version via `is_current=TRUE`.

## Description

Implement `src/patient_360/silver/transform_providers.py` per LLD §5.2 / DMS §6. Reads `unity.bronze.synthea_providers` (UC-managed), drops PHI columns per DMS §3 (LLD §5.2), applies derived fields, then calls `apply_scd2(...)` from `utils/scd2.py`. PHI fields like `SSN`, `DRIVERS`, `PASSPORT` (where applicable) MUST be dropped at this Silver boundary (LLD §5.2, NFR-6). Output table: `clinical_providers` for patients, `reference_providers` otherwise. Inline SE via `se_runner.run_dq(...)`.

## Acceptance Criteria


- [ ] `transform_providers.py` reads `unity.bronze.synthea_providers` (UC-managed; LLD §13 Decision 15) [LLD §5.2, §13]
...
- **Implementation hints**: Reuse `apply_scd2` from `utils/scd2.py` (STORY-01-003). Output target is `warehouse/{env}/silver/reference/reference_providers/` (SCD2 dims have no `ds` partition per LLD §3.3).
