...
- [ ] Intra-layer dependencies wired per LLD §4.2: `transform_encounters_silver` ← `transform_{patients,organizations,providers}_silver`; the six encounter-dependent facts + careplans ← `transform_encounters_silver`; `transform_allergies_silver` ← `transform_patients_silver` [LLD §4.2, §4.3]
