...
Each Bronze table is driven by a YAML config under
`airflow/configs/{table}.yml` and the generic runner
`src/patient_360/bronze/ingestion_runner.py`:

```bash
# One table, locally (DuckDB → unity.bronze.synthea_<table>)
cd patient_360
uv run python -m patient_360.bronze.ingestion_runner \
    --config-path airflow/configs/patients.yml \
    --env DEV --ds 2026-04-27
```

In the docker stack, the Airflow DAG `patient360_hourly_v1` calls the
TaskGroup factory in `src/patient_360/bronze/ingestion_factory.py`,
which scans `AIRFLOW_CONFIGS_DIR` and emits one SparkSubmitOperator
task per file.

## Run Bronze locally (full DAG against UC OSS)
...
airflow dags list-runs -d patient360_hourly_v1
curl -sS 'http://localhost:8080/api/2.1/unity-catalog/tables?catalog_name=unity&schema_name=bronze' | jq '.tables | length'
```

Expected output: `13` (one per Bronze table) plus SE-managed
`bronze_se_stats` and per-table `_error` tables.

## Verifying Bronze layer

The end-to-end integration test asserts UC presence + SE evidence +
