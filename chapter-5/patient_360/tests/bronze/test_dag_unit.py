...
def dag_module(monkeypatch_module, patched_spark_submit):
    ...
    monkeypatch_module.setenv(
        "AIRFLOW_CONFIGS_DIR", str(REPO_ROOT / "airflow" / "configs")
    )
    spec = importlib.util.spec_from_file_location(
        "patient360_hourly_v1_dag", DAG_FILE
    )
    assert spec is not None and spec.loader is not None, (
        f"Could not load DAG spec for {DAG_FILE}"
    )
...
def test_dag_file_exists():
    ...
    assert DAG_FILE.is_file(), (
        f"Expected DAG file at {DAG_FILE} — STORY-02-006 AC1"
    )
...
def test_dag_id_and_schedule(dag_module):
    ...
    schedule_repr = (
        getattr(dag, "schedule_interval", None) or repr(dag.timetable)
    )
    assert "0 * * * *" in str(schedule_repr) or "hourly" in str(
        schedule_repr
    ).lower()
...
def test_dag_exposes_14_bronze_layer_tasks(dag_module):
    ...
    bronze_task_ids = {
        t for t in task_ids if t.startswith("bronze_ingestion.ingest_")
    }
...
def test_reconciliation_downstream_of_bronze(dag_module):
    ...
    bronze_upstream = {
        t for t in upstream_ids if t.startswith("bronze_ingestion.ingest_")
    }
