...
# LLD §2.3 ingestion runner contract — three metadata columns are added
# to every Bronze table before the Delta write.
METADATA_COLUMNS: tuple[str, ...] = ("ds", "_ingested_at", "_source_batch_id")
...
def successful_dag_run(stack) -> dict[str, Any]:
    ...
    if payload.get("state") != "success":
        pytest.fail(
            f"DAG run {dag_id}/{run_id} ended in state {payload.get('state')!r}: "
            f"{payload}"
        )
    return payload
