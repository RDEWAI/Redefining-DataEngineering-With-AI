    ...
    if sub is None or not sub.strip():
        results.append(
            ValidationResult(
                ...
                suggestion=(
                    "Add at least one data consumer with"
                    " department, use case, and access pattern."
                ),
            )
        )
...
def check_regulatory_compliance(sections: dict[str, str]) -> list[ValidationResult]:
    ...
    regulatory_subsections = [
        (
            ...
            (
                "Identify regulations that apply"
                " (e.g., HIPAA, GDPR) with scope"
                " and impact on data design."
            ),
        ),
        (
    ...
    if found_patterns:
        ...
        results.append(
            ValidationResult(
                ...
                section="General",
                message=(
                    "Found vague language that should be"
                    f" made specific: {', '.join(found_patterns)}."
                ),
        ...
        results.append(
            ValidationResult(
                ...
                suggestion=(
                    ...
                    " requirements or mark as open questions."
                ),
            )
        )
    ...
    if tbd_without_details:
        results.append(
            ValidationResult(
                ...
                section="General",
                message=(
                    f"Found {len(tbd_without_details)}"
                    " [TO BE DETERMINED] placeholder(s)"
                    " missing owner or due date."
                ),
                suggestion=(
                    "Use format: [TO BE DETERMINED"
                    " - requires input from {Name},"
                    " due {YYYY-MM-DD}]."
                ),
            )
        )
