...
**Edit patterns:**
- **Modify existing content**: `Edit` with exact old text → new text
- **Insert new content**: `Edit` anchored on the line before the insertion point
- **Remove content**: `Edit` replacing content with empty string (only with user approval)
...
```bash
echo '{"skill": "update-drd", "date": "{YYYY-MM-DD}", "correction": "{what the user said or changed}", "pattern": "{generalized rule}", "status": "pending"}' >> memory/drd/learnings-queue.jsonl
```
