...
def generate_dag_yaml(
    ...
    section_5_tasks: list[dict[str, str]] | None = None,
) -> str:
    ...
    # Build a lookup from §5 task rows for scaffold paths (module/contract/dq_rules)
    section_5_by_id: dict[str, dict[str, str]] = {}
    if section_5_tasks:
        for row in section_5_tasks:
            tid = row.get("task id", "").strip("`").strip()
            if tid:
                section_5_by_id[tid] = row

    lines = [
        f"# DAG Definition: {dag_id}",
        f"# Derived from: {lld_filename}, Section 4 + Section 5",
        "# Generated by: generate_dag_definition.py",
        "",
        "dag:",
        f"  dag_id: {dag_id}",
        f"  dag_file: airflow/dags/{dag_id}.py",
        f"  config_file: airflow/configs/{dag_id}.yaml",
        f'  schedule: "{schedule}"',
    ...
    for task in tasks:
        ...
        layer = task.get("layer", "unknown").lower()

        s5 = section_5_by_id.get(task_id, {})
        module_path = s5.get("module path", "").strip("` ")
        contract_path = s5.get("contract file", "").strip("` ")
        dq_path = s5.get("dq rules file", "").strip("` ")
        dag_node = s5.get("dag task node", "").strip("` ")

        # Derive module from scaffold convention if not given
        if not module_path:
            module_path = f"src/{project_name}/{layer}/{task_id.lower()}.py"
        dotted_module = module_path.removeprefix("src/").removesuffix(".py").replace("/", ".")

        lines.append(f"  - task_id: {task_id}")
        if dag_node:
            lines.append(f"    dag_node: {dag_node}")
        lines.append(f"    type: {task.get('type', 'PySpark')}")
        lines.append(f"    layer: {layer}")
        lines.append(f"    module: {dotted_module}")
        if contract_path:
            lines.append(f"    contract: {contract_path}")
        if dq_path:
            lines.append(f"    dq_rules: {dq_path}")
...
def main() -> int:
    ...
    # Parse LLD header for Project Name / Chapter / Target Scaffold
    ...
    # Parse §5 task implementation table for scaffold paths (module / contract / dq_rules)
    section_5 = sections.get("5. Task Implementation Details", "")
    section_5_tasks = parse_task_table(section_5) if section_5 else []
    ...
    yaml_content = generate_dag_yaml(
        metadata,
        tasks,
        critical_path,
        args.path.name,
        project_name=project_name,
        section_5_tasks=section_5_tasks,
    )
