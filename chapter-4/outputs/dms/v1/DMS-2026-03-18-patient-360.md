...
| Gold | Denormalized consumer tables for 2-second SLA | 3 | Consumer-aligned grain, `ARRAY<STRUCT>` for allergies, role-segregated cost data |
...
Allergies are stored as `ARRAY<STRUCT<description VARCHAR, severity VARCHAR>>` to eliminate joins at query time. This complex type is conditionally approved in Gold per the updated enterprise data dictionary §1.
...
columns:
  ...
  - name: allergies
    ...
    nullable: true
    complex_type_rationale: "Eliminates a join to clinical_allergies at query time, critical for the 2-second search SLA [DRD §4.3]. Bridge table would require join for every patient lookup. Max expected cardinality ~3 allergies per patient (5,607 allergies / 5,767 patients)."
...
|-----------|-------------|-------------|---------------------|
| analytics.patient_summary | clinical.clinical_patients (is_current=true), clinical.clinical_encounters (agg), clinical.clinical_conditions (agg), clinical.clinical_medications (agg), clinical.clinical_allergies (all) | bronze.synthea_patients, bronze.synthea_encounters, bronze.synthea_conditions, bronze.synthea_medications, bronze.synthea_allergies | SCD2 current-version join; ARRAY<STRUCT> for allergies to avoid join at query time; no cost columns [DRD §5.5]; 1 row per patient [HLD FR-1] |
...
|---|----------|-------------|----------|--------|
| 1 | Should `patient_clinical_history` include a structured list of conditions per encounter (ARRAY<STRUCT>) or remain count-only? If condition descriptions are needed for display, a STRUCT would eliminate a Silver join. | Data Engineering Team | 2026-04-15 | Open |
