...
+-- patient_360/                                   # main Python project (the implementation)
    ...
    |   +-- bootstrap_se_tables.py                 # idempotent SE Delta-table CREATE (run by bootstrap_se task)
    |   +-- gen_liquibase_from_contracts.py        # v1.16 (Decision 17 v1.16) -- no longer required for UC bring-up; disposition (delete vs repurpose for Postgres audit) deferred to next update-scaffold
    |   +-- bootstrap_uc_tables.py                 # REVISED v1.18 (Decision 17 v1.18) -- pure-Python httpx client against UC OSS /api/2.1/unity-catalog/{schemas,tables}; idempotent via GET-before-POST; NO Spark, NO JDBC, NO UCSingleCatalog
...
| Schema migrations / DDL (Postgres audit-trail only — v1.17, unchanged v1.18) | `ddl/liquibase/changelogs/` (Postgres audit-trail role only; UC OSS table registration owned by `scripts/bootstrap_uc_tables.py` per Decision 17 v1.18 — pure-Python REST client) |
| Liquibase changelog generator (v1.16 — superseded for UC) | `scripts/gen_liquibase_from_contracts.py` (v1.16 — no longer required for UC bring-up under Decision 17 v1.17/v1.18; disposition deferred to next `update-scaffold`) |
| UC bootstrap REST client (REVISED v1.18) | `scripts/bootstrap_uc_tables.py` (Decision 17 v1.18) — pure-Python httpx client against UC OSS `/api/2.1/unity-catalog/{schemas,tables}` endpoints; reads `contracts/*.yml` directly; idempotent via GET-before-POST (lists existing schemas/tables first, only POSTs missing ones); **NO SparkSession, NO JDBC, NO UCSingleCatalog**. Invoked by `make bootstrap-uc`. Replaces the v1.17 Spark-DDL approach (binary-incompatible with Spark 4.x — see Decision 17 v1.18 block). |
...
- **Input**: Per-table YAML config path (resolved via `AIRFLOW_CONFIGS_DIR` env, default `${PATIENT360_PROJECT_ROOT}/airflow/configs`), Spark session built with the default `spark_catalog` backed by an embedded Hive metastore on Derby (see §13 Decision 12, revoked & replaced 2026-05-12), pipeline run context
- **Output**: Path-based Delta table at `${PATIENT360_PROJECT_ROOT}/warehouse/{env}/bronze/<table>/ds=<ds>/` (Decision 15, revoked 2026-05-12). The Unity Catalog OSS server runs as a UI-demo only and does NOT participate in the write path.
- **Responsibility**: Reads YAML config to determine source (CSV by default; DuckDB only for source tables < 100 MB — see §5.1), column contract, DQ rule references (resolved via `DQ_RULES_DIR` env, defaulting to `${PATIENT360_PROJECT_ROOT}/dq_rules`, then `dq_rules/{table}.yml` by table name convention), and empty-input behavior. Executes the standard ingestion pattern: read source, add metadata columns, enforce the source-derived column contract (no inference), call SE inline for row_dq/agg_dq validation, write via `df.write.mode("append").format("delta").partitionBy("ds").option("replaceWhere", "ds = '<ds>'").save("${PATIENT360_PROJECT_ROOT}/warehouse/{env}/bronze/<table>/")`.
...
- **Input**: `df: DataFrame` (incoming, DQ-validated records — caller MUST have already invoked `se_runner.run_dq` before calling this function), `target_path: str` (Delta table path, e.g. `warehouse/{env}/silver/clinical/clinical_patients/`), `natural_keys: list[str]`, `hash_columns: list[str]`, `effective_date: date`. The legacy `target_table` keyword argument is **removed** (Decision 17, 2026-05-23) — UC table registration is now deploy-time only; the helper never references catalog FQNs.
- **Output**: `dict[str, int]` — `{"rows_inserted": int, "rows_closed": int, "rows_unchanged": int}` — metrics derived from `DESCRIBE HISTORY` for the last operation; the caller emits these via OpenTelemetry per §5.4 step 8.
- **Responsibility**: Generic SCD2 merge using SHA-256 hash comparison and Delta MERGE INTO with matched/unmatched logic. The Spark session is derived internally via `df.sparkSession` (not a parameter). The helper trusts every row it receives — it MUST NOT call `run_dq` or any other DQ gate. **Path-based only — `apply_scd2` MUST NOT call `CREATE TABLE` under any circumstance** (Decision 17). The transitional `CREATE TABLE IF NOT EXISTS … USING DELTA LOCATION …` call added 2026-05-22 (see `chapter-6/developer-plugin/LLD-DEVIATIONS.md` row 10) is **revoked**; UC visibility is owned by `make bootstrap-uc` (Liquibase apply against UC JDBC), not by the runtime writer. **Cold-warehouse first-run handling stays**: if `_delta_log/` is absent under `target_path`, write the initial Delta version via `df.write.format("delta").mode("overwrite").save(target_path)` — same path-based pattern, no catalog DDL, no `MERGE INTO <fqn>`. Use `delta.\`<target_path>\`` MERGE syntax when a path-form MERGE is needed. Surrogate-key generation MUST be deterministic across re-runs — use `xxhash64(concat_ws('|', *natural_keys), effective_date)` OR read `max(surrogate_key)` from the target and increment via `row_number()`. NEVER use `monotonically_increasing_id()` (non-deterministic per executor; produces different keys on retry, breaking history). For SCD2 strategy details, see DMS §6.
...
**`scripts/bootstrap_uc_tables.py`** -- UC Bootstrap REST Client (REVISED v1.18, Decision 17 v1.18)
- **Input**: Environment selector (`--env dev|stage|prod`, defaults to `dev`); `contracts/*.yml` (every table contract under `${PATIENT360_PROJECT_ROOT}/contracts/` with a populated `columns:` list — the human-edited source of truth for column names + types + nullability). Resolves UC catalog name from `${UC_BOOTSTRAP_CATALOG_NAME}` (default `unity`) and the UC REST URI from `${UC_BOOTSTRAP_URI}` (default `http://localhost:8080` for host invocation; override to `http://unity-catalog:8080` for in-container invocation) per §7.1 catalog block. HTTP timeout from `${UC_BOOTSTRAP_TIMEOUT_SECONDS}` (default `30`). Resolves warehouse root from `${PATIENT360_WAREHOUSE_ROOT}` per §9.1 anchor rules.
- **Output**: Process exit code (0 = all REST calls succeeded; non-zero on any HTTP failure after all attempts logged). Side effect: every Bronze/Silver/Gold/reference Delta table declared in `contracts/*.yml` with a populated `columns:` list is registered as an `EXTERNAL` Delta table in UC OSS via `POST /api/2.1/unity-catalog/tables` with `data_source_format=DELTA` and `storage_location=${PATIENT360_WAREHOUSE_ROOT}/{env}/<layer>/<domain>/<table>/`. One `POST /api/2.1/unity-catalog/schemas` per distinct schema that is not already present. Idempotent across re-runs (GET-before-POST: lists existing schemas/tables first, only POSTs ones that are missing). Re-run produces a detailed `"<name> exists"` / `"<name> created"` log per object.
- **Responsibility**: Pure-Python httpx client (one-shot — invokes REST, exits). **NO SparkSession. NO JDBC. NO UCSingleCatalog. NO Maven coordinates.** Run via `python scripts/bootstrap_uc_tables.py --env <env>`; the only runtime dependencies are `httpx` (or stdlib `urllib.request`) and `pyyaml` (both already in project deps). Iterates `contracts/*.yml`; skips contracts with an empty `columns:` block (nothing to register). For each populated contract: (1) `GET /api/2.1/unity-catalog/schemas?catalog_name={catalog}` → collect existing schema names; if `contract.schema` not present, `POST /api/2.1/unity-catalog/schemas` with `{"name": schema, "catalog_name": catalog}`. (2) `GET /api/2.1/unity-catalog/tables?catalog_name={catalog}&schema_name={schema}` → collect existing table names; if `contract.table` not present, `POST /api/2.1/unity-catalog/tables` with the full payload — `name`, `catalog_name`, `schema_name`, `table_type="EXTERNAL"`, `data_source_format="DELTA"`, `columns=[{"name", "type_text", "type_name", "type_json", "position", "nullable", ...}]`, `storage_location`. Error handling: on `HTTPException`, log `ERROR` with table name + status + body, accumulate the failure, continue iterating remaining contracts, then re-raise (or exit non-zero) at the end — fail-loud at end so the operator sees every failure in one pass. **Runtime writers are FORBIDDEN from calling `CREATE TABLE`** — UC visibility is exclusively this script's responsibility (Decision 17). See §13 Decision 17 for the deploy-time vs runtime separation. **Note (v1.18 second pivot)**: this script replaces the v1.17 Spark-DDL approach. `io.unitycatalog:unitycatalog-spark_2.13:0.3.0` (the latest on Maven Central as of 2026-05-23) is built against Spark 3.5.x; the project runs PySpark 4.0.x. The `org.apache.spark.internal.LogKey` trait was restructured between Spark 3.5 and Spark 4.0; UCSingleCatalog raises `java.lang.NoSuchMethodError: 'void org.apache.spark.internal.LogKey.$init$(org.apache.spark.internal.LogKey)'` on the first `CREATE TABLE` call. No newer `unitycatalog-spark_2.13` version exists. The REST API is the ONLY UC integration path that works with the project's Spark 4.x stack today — verified working 2026-05-23 via `curl` smoke test against `/api/2.1/unity-catalog/{schemas,tables}` (POST creates, GET lists, DELETE cleans up). Decoupling UC registration from the Spark stack also lets UC OSS and the user's Spark/Delta versions evolve independently.
...
**Decision 17 deploy-time contract** (revised v1.18, 2026-05-23): every per-table path under `warehouse/{env}/<layer>/<domain>/<table>/` shown below IS the value passed as `storage_location` when the table is registered in UC OSS by `make bootstrap-uc` (`scripts/bootstrap_uc_tables.py` — pure-Python httpx client POSTing to `/api/2.1/unity-catalog/tables` with `table_type="EXTERNAL"` + `data_source_format="DELTA"`, per Decision 17 v1.18). The path is therefore load-bearing in two directions — runtime writers persist Delta data there (path-based `.save(<path>)` via DeltaCatalog, no `CREATE TABLE`), and the deploy-time bootstrap REST client registers an EXTERNAL Delta table with `storage_location` pointing at the same path. A drift between these two breaks UC visibility silently. The v1.16 Liquibase-against-UC-JDBC mechanism and the v1.17 Spark-DDL mechanism are both superseded — see §13 Decision 17 Revised v1.18 block for rationale (Spark 4 / UCSingleCatalog binary incompat reproduced 2026-05-23).
...
| Max active tasks | **DEV: 1 (re-capped 2026-05-21)**; STAGING: 8; PROD: 16 | DEV is re-capped to `max_active_tasks=1` because §13 Decision 12 selects an **embedded Derby Hive metastore**, which is a **single-process database** — only one JVM can open the metastore at a time. Reproduced 2026-05-21: with `max_active_tasks=16`, parallel `SparkSubmitOperator` tasks fail with `ERROR XSDB6: Another instance of Derby may have already booted the database null`. The `bootstrap_se` task (§4.2) only solves the SE-table CREATE race; it does NOT solve Derby's concurrent-access limit. The 2026-05-20 v1.14 raise from 1 → 16 was based on an incomplete read of the bottleneck. Concurrency > 1 in DEV requires migrating the metastore to a multi-client backend (candidate Decision 16 — Postgres-backed Hive Metastore Standalone, or reuse Marquez's existing Postgres). STAGING/PROD scale per infra (those environments are out of scope for the Derby constraint). |
| Concurrency | DEV: 1 (re-capped 2026-05-21); STAGING: 8; PROD: 16 | Mirrors `max_active_tasks`. See §13 Decision 12 Constraints paragraph for the Derby single-process rationale. |
| Default timeout | 60 min (DEV), 120 min (PROD) | Per infrastructure-specs.md §1.2 |

### 4.2 Task Inventory

**Spark-touching tasks MUST use `SparkSubmitOperator`** (mandatory, no exceptions). Airflow 3.x runs every task inside the scheduler/worker process by default; embedding `pyspark.sql.SparkSession.builder.getOrCreate()` inside a `PythonOperator` causes the Airflow worker JVM and the Spark driver JVM to collide on classloaders, native libraries, and `SPARK_HOME`, producing intermittent hangs and `JavaPackage object is not callable` failures that never reproduce locally outside Airflow. Every task in the table below whose `Type` mentions "Spark" or "PySpark" runs as a `SparkSubmitOperator` (in chapter-5, wrapped by the project's thin `spark_submit_wrapper.py`); reconciliation tasks that only execute SQL against Delta paths via the Hive metastore are exempt and use `PythonOperator` or `SQLExecuteQueryOperator`.

**Root bootstrap task** (`bootstrap_se`): A single one-shot SparkSubmitOperator that runs at the head of every DAG run, upstream of the entire `bronze_ingestion` TaskGroup. It executes `scripts/bootstrap_se_tables.py` (resolved via the `SE_BOOTSTRAP_APP` env var, defaulting to `/opt/patient_360/scripts/bootstrap_se_tables.py` inside the airflow container) under `spark_default` to idempotently CREATE the shared Spark Expectations Delta tables: `CREATE SCHEMA IF NOT EXISTS bronze` plus `CREATE TABLE IF NOT EXISTS bronze.bronze_se_stats USING DELTA LOCATION '...'` and the matching `bronze.bronze_se_errors`. The script is a no-op when both tables already exist.

**Rationale (bootstrap_se)**: Spark Expectations appends per-run stats to a SHARED `bronze.bronze_se_stats` Delta table. Without an upstream bootstrap, two SE-table-level failure modes co-existed: **(a) cold-start CREATE-race** — on a fresh metastore, the first Bronze SparkSubmit task to land races its parallel siblings on the `CREATE TABLE` for `bronze_se_stats`, producing intermittent Bronze task failures the moment `max_active_tasks > 1`; **(b) warm-restart non-empty location** — after a container or metastore restart with on-disk Delta data still present under the table location, the auto-CREATE in the first Bronze task fails with `DELTA_CREATE_TABLE_WITH_NON_EMPTY_LOCATION` (encountered 2026-05-20; see `chapter-6/developer-plugin/LLD-DEVIATIONS.md` row 6). The `bootstrap_se` task closes both SE-table modes by pre-creating the stats and errors tables exactly once per DAG run via idempotent `CREATE … IF NOT EXISTS` against the canonical Delta locations. Sibling Bronze tasks then only APPEND, never CREATE.

**Note (2026-05-21 correction)**: `bootstrap_se` alone is **not** sufficient to raise DEV concurrency. It removes the SE-table CREATE race but does **not** address Derby's single-process limitation on the Hive metastore (§13 Decision 12). The two mechanisms together — `bootstrap_se` (SE-table CREATE race) **and** `max_active_tasks=1` (Derby concurrency cap) — close both failure modes in DEV. Raising concurrency beyond 1 in DEV would require a separate multi-client metastore backend (candidate Decision 16). The 2026-05-20 v1.14 raise to 16 was reverted on 2026-05-21 after `ERROR XSDB6: Another instance of Derby may have already booted the database null` reproduced on the first DAG run.

**Bronze TaskGroup** (`bronze_ingestion`): Generated by the ingestion factory function (`src/patient_360/bronze/ingestion_factory.py`). The factory scans all YAML files in `airflow/configs/`, creates one SparkSubmitOperator wrapper task per file, and groups them into an Airflow TaskGroup. The TaskGroup respects `max_active_tasks` from §4.1 (DEV: 1, STAGING: 8, PROD: 16) — in DEV the 13 tasks serialize behind the Derby single-process cap (§13 Decision 12 Constraints); STAGING/PROD fan out per their respective metastore backends. **All tasks in this TaskGroup take `bootstrap_se` as an upstream dependency.**

| Task ID | Type | Layer | Dependencies | Timeout | Retries | Retry Delay | Config File |
|---------|------|-------|-------------|---------|---------|-------------|-------------|
| `bootstrap_se` | SparkSubmitOperator | Bootstrap | None (root) | 5 min | 1 | 60s fixed | `scripts/bootstrap_se_tables.py` (via `SE_BOOTSTRAP_APP` env var; default `/opt/patient_360/scripts/bootstrap_se_tables.py`). `conn_id=spark_default`; `driver_memory=1g`, `executor_memory=1g`, `executor_cores=1`, `num_executors=1`. Idempotent: `CREATE SCHEMA IF NOT EXISTS bronze` + `CREATE TABLE IF NOT EXISTS bronze.bronze_se_stats USING DELTA LOCATION '...'` (and same for `bronze_se_errors`). Downstream of: none. Upstream of: every task in `bronze_ingestion` TaskGroup. |
| `bronze_ingestion.ingest_patients` | SparkSubmitWrapper | Bronze | `bootstrap_se` | 30 min | 3 | 60s exp | `airflow/configs/patients.yml` |
| `bronze_ingestion.ingest_encounters` | SparkSubmitWrapper | Bronze | `bootstrap_se` | 30 min | 3 | 60s exp | `airflow/configs/encounters.yml` |
| `bronze_ingestion.ingest_conditions` | SparkSubmitWrapper | Bronze | `bootstrap_se` | 30 min | 3 | 60s exp | `airflow/configs/conditions.yml` |
| `bronze_ingestion.ingest_medications` | SparkSubmitWrapper | Bronze | `bootstrap_se` | 30 min | 3 | 60s exp | `airflow/configs/medications.yml` |
| `bronze_ingestion.ingest_observations` | SparkSubmitWrapper | Bronze | `bootstrap_se` | 30 min | 3 | 60s exp | `airflow/configs/observations.yml` |
| `bronze_ingestion.ingest_allergies` | SparkSubmitWrapper | Bronze | `bootstrap_se` | 30 min | 3 | 60s exp | `airflow/configs/allergies.yml` |
| `bronze_ingestion.ingest_immunizations` | SparkSubmitWrapper | Bronze | `bootstrap_se` | 30 min | 3 | 60s exp | `airflow/configs/immunizations.yml` |
| `bronze_ingestion.ingest_procedures` | SparkSubmitWrapper | Bronze | `bootstrap_se` | 30 min | 3 | 60s exp | `airflow/configs/procedures.yml` |
| `bronze_ingestion.ingest_claims` | SparkSubmitWrapper | Bronze | `bootstrap_se` | 30 min | 3 | 60s exp | `airflow/configs/claims.yml` |
| `bronze_ingestion.ingest_careplans` | SparkSubmitWrapper | Bronze | `bootstrap_se` | 30 min | 3 | 60s exp | `airflow/configs/careplans.yml` |
| `bronze_ingestion.ingest_organizations` | SparkSubmitWrapper | Bronze | `bootstrap_se` | 30 min | 3 | 60s exp | `airflow/configs/organizations.yml` |
| `bronze_ingestion.ingest_providers` | SparkSubmitWrapper | Bronze | `bootstrap_se` | 30 min | 3 | 60s exp | `airflow/configs/providers.yml` |
| `bronze_ingestion.ingest_payers` | SparkSubmitWrapper | Bronze | `bootstrap_se` | 30 min | 3 | 60s exp | `airflow/configs/payers.yml` |
...
|---------|-------|-------------|---------------|---------------|---------------|--------|---------|---------------|----------|
| `ingest_patients` | Bronze | src/patient_360/bronze/ingestion_runner.py (generic; see §2.3) | contracts/patients.yml | dq_rules/patients.yml | `bronze_ingestion.ingest_patients` (factory-generated) | synthea.patients (DuckDB read-only); ingestion config `airflow/configs/patients.yml` | `warehouse/{env}/bronze/synthea_patients/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | STM Tab:Source-to-Bronze (identical for all 13) | Inline SE: row_dq + agg_dq from `dq_rules/patients.yml` (DQ-FLD-001 to DQ-FLD-012); empty-input: **fail** -- required for downstream |
| `ingest_encounters` | Bronze | src/patient_360/bronze/ingestion_runner.py (generic; see §2.3) | contracts/encounters.yml | dq_rules/encounters.yml | `bronze_ingestion.ingest_encounters` (factory-generated) | synthea.encounters; ingestion config `airflow/configs/encounters.yml` | `warehouse/{env}/bronze/synthea_encounters/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | STM Tab:Source-to-Bronze (identical for all 13) | Inline SE: row_dq + agg_dq from `dq_rules/encounters.yml` (DQ-FLD-013 to DQ-FLD-021); empty-input: **fail** -- required for Gold |
| `ingest_conditions` | Bronze | src/patient_360/bronze/ingestion_runner.py (generic; see §2.3) | contracts/conditions.yml | dq_rules/conditions.yml | `bronze_ingestion.ingest_conditions` (factory-generated) | synthea.conditions; ingestion config `airflow/configs/conditions.yml` | `warehouse/{env}/bronze/synthea_conditions/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | STM Tab:Source-to-Bronze (identical for all 13) | Inline SE: row_dq + agg_dq from `dq_rules/conditions.yml` (DQ-FLD-022 to DQ-FLD-025); empty-input: write_empty (default) |
| `ingest_medications` | Bronze | src/patient_360/bronze/ingestion_runner.py (generic; see §2.3) | contracts/medications.yml | dq_rules/medications.yml | `bronze_ingestion.ingest_medications` (factory-generated) | synthea.medications; ingestion config `airflow/configs/medications.yml` | `warehouse/{env}/bronze/synthea_medications/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | STM Tab:Source-to-Bronze (identical for all 13) | Inline SE: row_dq + agg_dq from `dq_rules/medications.yml` (DQ-FLD-026 to DQ-FLD-030); empty-input: write_empty (default) |
| `ingest_observations` | Bronze | src/patient_360/bronze/ingestion_runner.py (generic; see §2.3) | contracts/observations.yml | dq_rules/observations.yml | `bronze_ingestion.ingest_observations` (factory-generated) | synthea.observations; ingestion config `airflow/configs/observations.yml` | `warehouse/{env}/bronze/synthea_observations/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | STM Tab:Source-to-Bronze (identical for all 13) | Inline SE: row_dq + agg_dq from `dq_rules/observations.yml` (DQ-FLD-031 to DQ-FLD-033); empty-input: write_empty (default) |
| `ingest_allergies` | Bronze | src/patient_360/bronze/ingestion_runner.py (generic; see §2.3) | contracts/allergies.yml | dq_rules/allergies.yml | `bronze_ingestion.ingest_allergies` (factory-generated) | synthea.allergies; ingestion config `airflow/configs/allergies.yml` | `warehouse/{env}/bronze/synthea_allergies/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | STM Tab:Source-to-Bronze (identical for all 13) | Inline SE: row_dq + agg_dq from `dq_rules/allergies.yml` (DQ-FLD-034 to DQ-FLD-035); empty-input: **fail** -- safety critical [DRD §1.3] |
| `ingest_immunizations` | Bronze | src/patient_360/bronze/ingestion_runner.py (generic; see §2.3) | contracts/immunizations.yml | dq_rules/immunizations.yml | `bronze_ingestion.ingest_immunizations` (factory-generated) | synthea.immunizations; ingestion config `airflow/configs/immunizations.yml` | `warehouse/{env}/bronze/synthea_immunizations/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | STM Tab:Source-to-Bronze (identical for all 13) | Inline SE: row_dq + agg_dq from `dq_rules/immunizations.yml` (DQ-FLD-038 to DQ-FLD-039); empty-input: write_empty (default) |
| `ingest_procedures` | Bronze | src/patient_360/bronze/ingestion_runner.py (generic; see §2.3) | contracts/procedures.yml | dq_rules/procedures.yml | `bronze_ingestion.ingest_procedures` (factory-generated) | synthea.procedures; ingestion config `airflow/configs/procedures.yml` | `warehouse/{env}/bronze/synthea_procedures/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | STM Tab:Source-to-Bronze (identical for all 13) | Inline SE: row_dq + agg_dq from `dq_rules/procedures.yml` (DQ-FLD-036 to DQ-FLD-037); empty-input: write_empty (default) |
| `ingest_claims` | Bronze | src/patient_360/bronze/ingestion_runner.py (generic; see §2.3) | contracts/claims.yml | dq_rules/claims.yml | `bronze_ingestion.ingest_claims` (factory-generated) | synthea.claims; ingestion config `airflow/configs/claims.yml` | `warehouse/{env}/bronze/synthea_claims/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | STM Tab:Source-to-Bronze (identical for all 13) | Inline SE: row_dq + agg_dq from `dq_rules/claims.yml` (DQ-FLD-040 to DQ-FLD-041); empty-input: write_empty (default) |
| `ingest_careplans` | Bronze | src/patient_360/bronze/ingestion_runner.py (generic; see §2.3) | contracts/careplans.yml | dq_rules/careplans.yml | `bronze_ingestion.ingest_careplans` (factory-generated) | synthea.careplans; ingestion config `airflow/configs/careplans.yml` | `warehouse/{env}/bronze/synthea_careplans/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | STM Tab:Source-to-Bronze (identical for all 13) | Inline SE: row_dq + agg_dq from `dq_rules/careplans.yml` (DQ-FLD-042); empty-input: write_empty (default) |
| `ingest_organizations` | Bronze | src/patient_360/bronze/ingestion_runner.py (generic; see §2.3) | contracts/organizations.yml | dq_rules/organizations.yml | `bronze_ingestion.ingest_organizations` (factory-generated) | synthea.organizations; ingestion config `airflow/configs/organizations.yml` | `warehouse/{env}/bronze/synthea_organizations/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | STM Tab:Source-to-Bronze (identical for all 13) | Inline SE: row_dq + agg_dq from `dq_rules/organizations.yml` (DQ-FLD-043); empty-input: **fail** -- FK dimension |
| `ingest_providers` | Bronze | src/patient_360/bronze/ingestion_runner.py (generic; see §2.3) | contracts/providers.yml | dq_rules/providers.yml | `bronze_ingestion.ingest_providers` (factory-generated) | synthea.providers; ingestion config `airflow/configs/providers.yml` | `warehouse/{env}/bronze/synthea_providers/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | STM Tab:Source-to-Bronze (identical for all 13) | Inline SE: row_dq + agg_dq from `dq_rules/providers.yml` (DQ-FLD-044); empty-input: **fail** -- FK dimension |
| `ingest_payers` | Bronze | src/patient_360/bronze/ingestion_runner.py (generic; see §2.3) | contracts/payers.yml | dq_rules/payers.yml | `bronze_ingestion.ingest_payers` (factory-generated) | synthea.payers; ingestion config `airflow/configs/payers.yml` | `warehouse/{env}/bronze/synthea_payers/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | STM Tab:Source-to-Bronze (identical for all 13) | Inline SE: row_dq + agg_dq from `dq_rules/payers.yml` (DQ-FLD-045); empty-input: **fail** -- FK dimension |
...
> **Note on the "DQ Check" column.** The cell text combines two independent
> gates that the per-table YAML config keeps separate. Read the cell as the
> union of:
>
> - `empty_input_behavior` (one of `fail` / `write_empty`) — what to do
>   when the upstream source is empty for `ds`. Evaluated BEFORE any
>   transformation runs.
> - `se_action_if_failed` (one of `fail` / `drop` / `ignore`) — how SE
>   responds to row_dq/agg_dq violations. Evaluated AFTER transformation,
>   BEFORE write. Per-rule `action_if_failed` declarations take
>   precedence over this fail-closed default.
>
> Critical tables (patients, encounters, allergies, FK dimensions) set
> `empty_input_behavior: fail` AND `se_action_if_failed: fail`. Non-critical
> clinical tables typically use `empty_input_behavior: write_empty` AND
> `se_action_if_failed: drop` (route rejected rows to quarantine, write
> the rest).

| Task ID | Layer | Module Path | Contract File | DQ Rules File | DAG Task Node | Inputs | Outputs | Transform Ref | DQ Check |
|---------|-------|-------------|---------------|---------------|---------------|--------|---------|---------------|----------|
| `transform_patients_silver` | Silver | src/patient_360/silver/transform_patients.py | contracts/clinical_patients.yml | dq_rules/clinical_patients.yml | `transform_patients_silver` (group: silver_dimensions or silver_facts) | `warehouse/{env}/bronze/synthea_patients/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | `warehouse/{env}/silver/clinical/clinical_patients/` | STM Tab:Bronze-to-Silver (patients) | Inline SE: DQ-FLD-046 to DQ-FLD-059, DQ-FLD-102 to DQ-FLD-104 from `dq_rules/clinical_patients.yml`; empty-input: Fail task -- patients required |
| `transform_encounters_silver` | Silver | src/patient_360/silver/transform_encounters.py | contracts/clinical_encounters.yml | dq_rules/clinical_encounters.yml | `transform_encounters_silver` (group: silver_dimensions or silver_facts) | `warehouse/{env}/bronze/synthea_encounters/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | `warehouse/{env}/silver/clinical/clinical_encounters/` | STM Tab:Bronze-to-Silver (encounters) | Inline SE: DQ-FLD-060 to DQ-FLD-065 from `dq_rules/clinical_encounters.yml`; empty-input: Fail task -- encounters required |
| `transform_conditions_silver` | Silver | src/patient_360/silver/transform_conditions.py | contracts/clinical_conditions.yml | dq_rules/clinical_conditions.yml | `transform_conditions_silver` (group: silver_dimensions or silver_facts) | `warehouse/{env}/bronze/synthea_conditions/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | `warehouse/{env}/silver/clinical/clinical_conditions/` | STM Tab:Bronze-to-Silver (conditions) | Inline SE: DQ-FLD-066 to DQ-FLD-070 from `dq_rules/clinical_conditions.yml`; empty-input: Write empty -- conditions optional |
| `transform_medications_silver` | Silver | src/patient_360/silver/transform_medications.py | contracts/clinical_medications.yml | dq_rules/clinical_medications.yml | `transform_medications_silver` (group: silver_dimensions or silver_facts) | `warehouse/{env}/bronze/synthea_medications/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | `warehouse/{env}/silver/clinical/clinical_medications/` | STM Tab:Bronze-to-Silver (medications) | Inline SE: DQ-FLD-071 to DQ-FLD-076 from `dq_rules/clinical_medications.yml`; empty-input: Write empty -- medications optional |
| `transform_observations_silver` | Silver | src/patient_360/silver/transform_observations.py | contracts/clinical_observations.yml | dq_rules/clinical_observations.yml | `transform_observations_silver` (group: silver_dimensions or silver_facts) | `warehouse/{env}/bronze/synthea_observations/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | `warehouse/{env}/silver/clinical/clinical_observations/` | STM Tab:Bronze-to-Silver (observations) | Inline SE: DQ-FLD-077 to DQ-FLD-079 from `dq_rules/clinical_observations.yml`; empty-input: Write empty -- observations optional |
| `transform_allergies_silver` | Silver | src/patient_360/silver/transform_allergies.py | contracts/clinical_allergies.yml | dq_rules/clinical_allergies.yml | `transform_allergies_silver` (group: silver_dimensions or silver_facts) | `warehouse/{env}/bronze/synthea_allergies/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | `warehouse/{env}/silver/clinical/clinical_allergies/` | STM Tab:Bronze-to-Silver (allergies) | Inline SE: DQ-FLD-080 to DQ-FLD-083 from `dq_rules/clinical_allergies.yml`; empty-input: Fail task -- safety critical [DRD §1.3] |
| `transform_immunizations_silver` | Silver | src/patient_360/silver/transform_immunizations.py | contracts/clinical_immunizations.yml | dq_rules/clinical_immunizations.yml | `transform_immunizations_silver` (group: silver_dimensions or silver_facts) | `warehouse/{env}/bronze/synthea_immunizations/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | `warehouse/{env}/silver/clinical/clinical_immunizations/` | STM Tab:Bronze-to-Silver (immunizations) | Inline SE: DQ-FLD-088 to DQ-FLD-090 from `dq_rules/clinical_immunizations.yml`; empty-input: Write empty |
| `transform_procedures_silver` | Silver | src/patient_360/silver/transform_procedures.py | contracts/clinical_procedures.yml | dq_rules/clinical_procedures.yml | `transform_procedures_silver` (group: silver_dimensions or silver_facts) | `warehouse/{env}/bronze/synthea_procedures/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | `warehouse/{env}/silver/clinical/clinical_procedures/` | STM Tab:Bronze-to-Silver (procedures) | Inline SE: DQ-FLD-084 to DQ-FLD-087 from `dq_rules/clinical_procedures.yml`; empty-input: Write empty |
| `transform_claims_silver` | Silver | src/patient_360/silver/transform_claims.py | contracts/billing_claims.yml | dq_rules/billing_claims.yml | `transform_claims_silver` (group: silver_dimensions or silver_facts) | `warehouse/{env}/bronze/synthea_claims/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | `warehouse/{env}/silver/billing/billing_claims/` | STM Tab:Bronze-to-Silver (claims) | Inline SE: DQ-FLD-093 to DQ-FLD-094 from `dq_rules/billing_claims.yml`; empty-input: Write empty |
| `transform_careplans_silver` | Silver | src/patient_360/silver/transform_careplans.py | contracts/clinical_careplans.yml | dq_rules/clinical_careplans.yml | `transform_careplans_silver` (group: silver_dimensions or silver_facts) | `warehouse/{env}/bronze/synthea_careplans/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | `warehouse/{env}/silver/clinical/clinical_careplans/` | STM Tab:Bronze-to-Silver (careplans) | Inline SE: DQ-FLD-091 to DQ-FLD-092 from `dq_rules/clinical_careplans.yml`; empty-input: Write empty |
| `transform_organizations_silver` | Silver | src/patient_360/silver/transform_organizations.py | contracts/reference_organizations.yml | dq_rules/reference_organizations.yml | `transform_organizations_silver` (group: silver_dimensions or silver_facts) | `warehouse/{env}/bronze/synthea_organizations/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | `warehouse/{env}/silver/reference/reference_organizations/` | STM Tab:Bronze-to-Silver (organizations) | Inline SE: DQ-FLD-095 to DQ-FLD-096 from `dq_rules/reference_organizations.yml`; empty-input: Fail task -- FK dimension |
| `transform_providers_silver` | Silver | src/patient_360/silver/transform_providers.py | contracts/reference_providers.yml | dq_rules/reference_providers.yml | `transform_providers_silver` (group: silver_dimensions or silver_facts) | `warehouse/{env}/bronze/synthea_providers/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | `warehouse/{env}/silver/reference/reference_providers/` | STM Tab:Bronze-to-Silver (providers) | Inline SE: DQ-FLD-097 to DQ-FLD-099 from `dq_rules/reference_providers.yml`; empty-input: Fail task -- FK dimension |
| `transform_payers_silver` | Silver | src/patient_360/silver/transform_payers.py | contracts/reference_payers.yml | dq_rules/reference_payers.yml | `transform_payers_silver` (group: silver_dimensions or silver_facts) | `warehouse/{env}/bronze/synthea_payers/` (path-based Delta; Decision 12 revoked + Decision 15 revoked, 2026-05-12) | `warehouse/{env}/silver/reference/reference_payers/` | STM Tab:Bronze-to-Silver (payers) | Inline SE: DQ-FLD-100 to DQ-FLD-101 from `dq_rules/reference_payers.yml`; empty-input: Fail task -- FK dimension |
...
| `catalog_uc_uri` | string | `http://unity-catalog:8080` | UC OSS service URI consumed by the Bronze runner via the `UC_URI` env var | Yes |
| `catalog_bronze_catalog_name` | string | `unity` | UC catalog name housing the Bronze schema | No |
| `catalog_bronze_schema` | string | `bronze` | UC schema name for Bronze Delta tables | No |
| `uc.bootstrap_catalog_name` | string | `unity` | UC catalog name the bootstrap REST client targets (`catalog_name` query param on `GET /schemas` and `POST /schemas|/tables` payloads). Kept separate from runtime `spark_catalog` (Decision 17 v1.18). Env var: `UC_BOOTSTRAP_CATALOG_NAME`. Consumed exclusively by `scripts/bootstrap_uc_tables.py`. | No |
| `uc.bootstrap_uri` | string | `http://localhost:8080` | UC OSS REST base URL the bootstrap client POSTs/GETs against (Decision 17 v1.18). Default is `http://localhost:8080` for **host** invocation; override to `http://unity-catalog:8080` for **in-container** invocation (the `unity-catalog` hostname only resolves inside the compose network — v1.17's default of `http://unity-catalog:8080` failed for host bootstrap with `UnresolvedAddressException`). Env var: `UC_BOOTSTRAP_URI`. Consumed exclusively by `scripts/bootstrap_uc_tables.py`. | Yes |
| `uc.bootstrap_timeout_seconds` | int | `30` | HTTP client timeout per REST request issued by `scripts/bootstrap_uc_tables.py` (Decision 17 v1.18). Env var: `UC_BOOTSTRAP_TIMEOUT_SECONDS`. | No |
...
| Row-level DQ rejection (row_dq `action_if_failed: drop`) | Spark Expectations | `<target>_error` Delta table (auto-written by SE) | §8.2 |
| Aggregate/query DQ outcome (agg_dq, query_dq) | Spark Expectations | `*_se_stats` + `*_agg_dq_detailed` / `*_query_dq_detailed` Delta tables | §8.3 |
...
For every Bronze and Silver table that runs row-level SE validation, Spark Expectations automatically creates and writes to a companion **Delta error table** named `<target>_error`. This table is the single destination for any row that fails a `row_dq` rule with `action_if_failed: drop`. **The SE-managed `_error` table is the only destination for row_dq drops** — adding a parallel implementation would duplicate SE behavior and create two divergent sources of truth.
...
|---|---|---|---|
| `bronze_se_stats` | Delta (append) | Per-run aggregate stats: rules evaluated, pass/fail counts, row counts, runtime; one row per (table, run_id, rule_type) | SE stats writer |
| `silver_se_stats` | Delta (append) | Same schema as `bronze_se_stats` for Silver-layer runs | SE stats writer |
| `gold_se_stats` | Delta (append) | Same schema for Gold reconciliation query_dq runs | SE stats writer |
...
| `stats_table_name` | `{layer}_se_stats` | One stats table per layer (bronze/silver/gold) |
...
|----------|----------|----------|--------------|
| `bronze_se_stats` row keyed by `meta_dq_run_date = <ds>` | SE `se.enable.stats.table=true` (auto-written by `with_expectations`) | reconciliation_bronze | `reconciliation_bronze` queries this table at end of layer; if no row exists for the current `meta_dq_run_date`, the task fails-closed with `SE_RUN_MISSING_FOR_DS=<ds>` |
| `<table>_error` Delta table (or empty for clean runs) | SE `se.enable.error.table=true` | DQ dashboards | Absent table is a runtime fault; reconciliation surfaces it as CRITICAL |
...
`reconciliation_bronze` MUST contain a query equivalent to:

```sql
SELECT count(*) FROM bronze_se_stats
 WHERE meta_dq_run_date = '${ds}'
```

and fail-closed if the count is 0. Do NOT add `AND meta_dq_run_id = ...` — SE owns the run_id and the gate cannot predict it. This eliminates the silent-DQ failure mode without re-introducing the false-failure mode. The bootstrap story's `## How to Test (User)` block exercises this gate end-to-end (`make smoke-se` or pytest equivalent) so a human reviewer SEES `bronze_se_stats >= 1 row` before approving Done. The scrum-master rules `STORIES-SE-COVERAGE-001` and `STORIES-INTEGRATION-SE-001` enforce this requirement at backlog generation time; a backlog without the SE run-evidence ACs is rejected before the developer-plugin reads it.
...
| `ddl/liquibase/` | Schema migrations -- Liquibase changelogs (Postgres audit-trail role only, restored in v1.17 and unchanged in v1.18 — the v1.16 "JDBC_FLAVOR=uc" Liquibase-against-UC-JDBC path remains removed). UC OSS table registration is owned by `scripts/bootstrap_uc_tables.py` (**pure-Python REST client** — Decision 17 v1.18, replacing the v1.17 Spark-DDL approach which proved binary-incompatible with Spark 4.x), not by Liquibase. The disposition of `scripts/gen_liquibase_from_contracts.py` introduced by v1.16 (delete vs repurpose for Postgres audit) is deferred to the next `update-scaffold` pass. | Data Engineering | `ddl/liquibase/liquibase.properties`, `ddl/liquibase/changelogs/`, `ddl/liquibase/master-changelog.xml` |
...
**Deploy-time UC table registration (Decision 17 v1.18, 2026-05-23)**: Tables are registered by `scripts/bootstrap_uc_tables.py` — a **pure-Python httpx client** invoked via `make bootstrap-uc`. The script reads `contracts/*.yml` directly and issues REST calls against UC OSS: `GET /api/2.1/unity-catalog/schemas?catalog_name=${UC_BOOTSTRAP_CATALOG_NAME}` (list existing), `POST /api/2.1/unity-catalog/schemas` (create missing schemas), `GET /api/2.1/unity-catalog/tables?catalog_name=…&schema_name=…` (list existing), `POST /api/2.1/unity-catalog/tables` with `table_type="EXTERNAL"`, `data_source_format="DELTA"`, full `columns` payload, and `storage_location="<warehouse_root>/<env>/<layer>/<domain>/<table>/"` (create missing tables). Idempotent via GET-before-POST. **No JVM required.** The bootstrap container only needs Python 3.12 + `httpx` (or stdlib `urllib.request`) + `pyyaml`. The v1.17 Spark-DDL mechanism (one-shot SparkSession with `UCSingleCatalog` DDL-only) is superseded: `unitycatalog-spark_2.13:0.3.0` is built against Spark 3.5.x and binary-incompatible with the project's PySpark 4.0.x runtime — `CREATE TABLE` raises `java.lang.NoSuchMethodError: 'void org.apache.spark.internal.LogKey.$init$(org.apache.spark.internal.LogKey)'`, reproduced 2026-05-23 with `make bootstrap-uc`. No newer `unitycatalog-spark_2.13` version is published. REST API verified working same-day via `curl` smoke (POST creates a schema, GET lists it, DELETE removes it). The v1.16 Liquibase-against-UC-JDBC mechanism remains superseded (`io.unitycatalog:unitycatalog-jdbc` is not published on Maven Central). Liquibase remains in the pre-v1.16 Postgres-only audit-trail role. Runtime writers remain DeltaCatalog + path-based `.save(<absolute-path>)`; no runtime `CREATE TABLE` — UC visibility is a **deploy-time invariant**, not a runtime one (Decision 12: UC is a non-blocking dependency at runtime).
...
|---|---|---|---|
| `unity-catalog` | `unitycatalog/unitycatalog:v0.4.0` | Unity Catalog OSS server -- catalog/metastore for Delta tables | 8080 |
| `unity-catalog-ui` | **build from source** (`_infra/docker/uc-source/ui/`, pinned to upstream tag `v0.4.0`) | Unity Catalog browser UI -- mirrors upstream `compose.yaml` build pattern | 3000 |

> **Upstream-reality note (v1.10):** the Unity Catalog project does not publish a versioned Docker image for its UI — only the server image is released to Docker Hub. The official `compose.yaml` at `github.com/unitycatalog/unitycatalog/compose.yaml` builds the UI from `ui/` in the source tree (`build.context: ui/`). We mirror that: `_infra/docker/uc-source/` is a shallow clone of `unitycatalog/unitycatalog` pinned to tag `v0.4.0`, and the `unity-catalog-ui` compose service declares `build: { context: _infra/docker/uc-source/ui/, dockerfile: Dockerfile }`. Port mapping changes from `3001` to `3000` to match the upstream UI default. A prior pin to `unitycatalog/unitycatalog-ui:v0.4.0` failed at `docker compose up` because that tag does not exist on Docker Hub (only `main` / `main-aadc6fc` rolling tags exist).
>
...
| `airflow` | local-built (`Dockerfile.airflow`) | Airflow 3.2.1 standalone (LocalExecutor; Spark embedded as `local[2]`) | 8081 |
...
| `make bootstrap-uc` (REVISED v1.18, Decision 17 v1.18) | `python scripts/bootstrap_uc_tables.py --env $(ENV)` (**pure-Python httpx client** — reads `contracts/*.yml`, GETs existing schemas/tables under `${UC_BOOTSTRAP_CATALOG_NAME}`, POSTs missing ones as EXTERNAL Delta tables with `storage_location=<warehouse_root>/<env>/<layer>/<domain>/<table>/`) | Deploy-time: register every populated contract as an external Delta table in UC OSS. Idempotent via GET-before-POST. Required after `make dev-up` and after any contract change. Chain into `dev-up` as a prerequisite to guarantee a fresh stack is UC-visible end-to-end. **No Spark, no JDBC, no JVM** — only Python + httpx + pyyaml. Replaces the v1.17 Spark-DDL approach (binary-incompatible with Spark 4.x) and the v1.16 Liquibase-against-UC-JDBC chain (UC JDBC driver not on Maven Central). |
...
| Unity Catalog OSS | HLD §5.1 | Schema registration; `spark_catalog` default; local REST API at `http://localhost:8080/api/2.1/unity-catalog` for all envs [infrastructure-specs.md §4] | §1, §7.1, §8.1 (circuit breaker), §9.5 |
...
### Decision 12: Local-Only Architecture with Delta + Hive Metastore (Derby) — UC OSS Demoted to UI Demo Only

**Status**: Revoked & Replaced (2026-05-12). The prior selection (`UCSingleCatalog` as the Spark catalog implementation for all environments) was reversed after a debug session demonstrated that UCSingleCatalog's commit path is incompatible with the local-FS, single-laptop Airflow + Spark embedded runtime used in chapter-5. Bronze runners hung indefinitely on `saveAsTable` against the UCSingleCatalog provider, and the failure mode did not surface in unit tests (which mock the catalog). The replacement decision below is the new contract.

**Options Considered (revised)**:
1. Keep UCSingleCatalog as the Spark catalog for all environments (original Decision 12; revoked).
2. Use the default Spark catalog backed by an embedded Hive metastore (Derby) on the local filesystem; keep the Unity Catalog OSS server running purely as a UI-demo / browsing surface that an operator can point at the same `warehouse/{env}/` directory.
3. Drop Unity Catalog entirely (no UI, no server, no demo).

**Selected**: Option 2 (Delta + Hive metastore on Derby for the runtime path; UC OSS retained for the UI demo only).

**Rationale**: chapter-5 is an educational, single-laptop deployment. The runtime catalog needs to be: (a) zero-configuration on a fresh checkout, (b) survive Airflow worker restarts without orchestration, and (c) work in-process with Spark embedded inside the Airflow worker (`local[*]`). An embedded Hive metastore on Derby satisfies all three with the Spark + Delta default wiring (`spark.sql.catalog.spark_catalog=org.apache.spark.sql.delta.catalog.DeltaCatalog`, `javax.jdo.option.ConnectionURL=jdbc:derby:metastore_db;create=true`). UC OSS remains valuable as a teaching surface — operators can browse the warehouse, see schemas, and demo the catalog UI — so the `unity-catalog` and `unity-catalog-ui` compose services stay, but they are no longer in the read/write path of the pipeline.

**Trade-off**: Bronze/Silver/Gold tables are no longer first-class objects in Unity Catalog at write time. Operators who want to browse them in the UC UI must either point UC at the same `warehouse/{env}/` directory or run a manual one-shot registration. Acceptable because chapter-5's primary teaching goal is the Medallion pipeline itself, not UC integration. A future chapter can re-introduce UC-managed writes once the upstream UCSingleCatalog provider is stable for embedded Spark.

**Constraints (added 2026-05-21)**: Embedded Derby is a **single-process** database — only one JVM at a time may open the metastore database. Any DAG running multiple `SparkSubmitOperator` tasks against this metastore MUST cap `max_active_tasks` at **1 in DEV** (§4.1). Concurrency > 1 requires migrating the metastore to a multi-client backend (e.g. Postgres-backed Hive Metastore Standalone, or reuse Marquez's existing Postgres instance) — tracked as candidate Decision 16 below. This single-process limit is the trade-off accepted in exchange for a zero-config local stack. Reproduced 2026-05-21: with `max_active_tasks=16`, the second `SparkSubmitOperator` to start raises `ERROR XSDB6: Another instance of Derby may have already booted the database null` and the DAG run fails. Note: the `bootstrap_se` task (§4.2) only solves the SE-table CREATE race; it does NOT bypass Derby's concurrent-access limit.

**Affected sections**: §1 (Design Overview — UC OSS now "UI demo only"), §2.3 (Bronze runner contract reverts to path-based Delta writes against `warehouse/{env}/bronze/{table}/`), §3.1 (storage format — drop "registered in UC"), §4.1 (DEV `max_active_tasks=1` per Derby constraint above), §5.1 (Output column reverts to `warehouse/{env}/bronze/{table}/`), §7 (config schema — catalog block redirected to Hive metastore parameters), §8.1 (circuit breaker — UC OSS becomes a non-blocking dependency like Marquez/Grafana), §9.5 (health checks — UC health degraded to WARNING), §13 Decision 15 (revoked; see Decision 15 update), §13 Decision 16 candidate (Postgres metastore migration for DEV concurrency > 1).
...
### Decision 15: Bronze Writes — Reverted to Path-Based Delta (Decision 12 Revocation Follow-Through)

**Status**: Revoked (2026-05-12). The UC-managed Bronze write path required by the original Decision 15 is no longer reachable now that Decision 12 has been revoked: there is no `UCSingleCatalog`-backed Spark catalog at runtime. Bronze writes revert to path-based Delta against `warehouse/{env}/bronze/<table>/`.

**Revised Selection**: Path-based Delta writes — `df.write.mode("append").format("delta").partitionBy("ds").option("replaceWhere", "ds = '<ds>'").save("warehouse/{env}/bronze/<table>/")`. This matches what the runner used before Decision 15 was introduced. Silver and Gold also use path-based Delta against the same `warehouse/{env}/` root.

**Rationale**: With Decision 12 reverted to Delta + Hive (Derby), the `unity.bronze.<table>` namespace no longer exists in the runtime catalog. The previous `validate-dag UC-WIRING-001` rule that rejected path-based Bronze is now incorrect for chapter-5 and will be retired by the developer-plugin in lock-step with this LLD revision. UC OSS remains running for the UI demo (Decision 12) but does not participate in writes.

**Trade-off**: Bronze tables are no longer discoverable in the UC UI without a manual external-table registration step. Acceptable — chapter-5's teaching goal is the Medallion pipeline mechanics, not UC integration. The recurring "Delta files invisible in UC" gap that motivated the original Decision 15 is now an expected outcome; operators who want UC browsing run a one-shot registration script (out of scope of the pipeline runtime).

**Affected sections**: §2.3 (Bronze runner contract — `.save("warehouse/{env}/bronze/<table>/")` replaces `.saveAsTable("unity.bronze.<table>")`), §5.1 (Output column reverts to `warehouse/{env}/bronze/<table>/`), §7 (catalog block redirected to Hive metastore parameters; `catalog_uc_uri` / `catalog_bronze_catalog_name` / `catalog_bronze_schema` deprecated for runtime use, retained only for UC UI demo).
...
### Decision 17: Contracts-as-Source + Deploy-Time UC Table Registration

**Status**: Pending Review (Revised v1.18 — 2026-05-23). **SECOND PIVOT.** The v1.17 selection (Option 4 — one-shot Spark session with `UCSingleCatalog` DDL-only) is **superseded** by the new v1.18 block immediately below. The v1.17 block is preserved underneath for audit. The original v1.16 selection (Option 3 — Liquibase against UC OSS via `io.unitycatalog:unitycatalog-jdbc`) remains preserved at the bottom of this decision for audit (struck-through quarantine — `~~text~~`). Decision 17 now carries three stacked blocks (newest first): **v1.18 REST API (active)** → v1.17 Spark-DDL (superseded, preserved for audit) → v1.16 Liquibase UC JDBC (superseded, preserved for audit).

---

#### Revised 2026-05-23 (v1.18) — Option A: Pure-Python REST Client (ACTIVE)

**Pivot Rationale**: The v1.17 selection (Option 4 — one-shot Spark session with `UCSingleCatalog` DDL-only) is **not executable today either**. Reproduced 2026-05-23 with `make bootstrap-uc`:

```
java.lang.NoSuchMethodError: 'void org.apache.spark.internal.LogKey.$init$(org.apache.spark.internal.LogKey)'
  at io.unitycatalog.spark.UCProxy (delta 4.0 ScalaModule)
  triggered by: CREATE TABLE IF NOT EXISTS unity.silver.clinical_patients USING DELTA LOCATION '...'
```

Root cause: `io.unitycatalog:unitycatalog-spark_2.13:0.3.0` (the latest published artifact on Maven Central, dated 2025-06-06) is built against Spark 3.5.x. The project runs PySpark 4.0.x. The `org.apache.spark.internal.LogKey` trait was restructured between Spark 3.5 and Spark 4.0; UCSingleCatalog calls a method that no longer exists. No newer `unitycatalog-spark_2.13` version is published (0.2.0, 0.2.1, 0.3.0 — all pre-Spark-4). This is the **same root-cause family** that motivated Decision 12 in 2026-05-12 (`UCSingleCatalog` unreliable for embedded Spark) — the v1.17 plan underestimated that even DDL-only operations hit the same Spark 4 wall.

**Validation of new option**: REST API verified working 2026-05-23. Smoke test:

```bash
curl -X POST http://localhost:8080/api/2.1/unity-catalog/schemas \
  -H 'Content-Type: application/json' \
  -d '{"name": "_bootstrap_test", "catalog_name": "unity"}'
```

returned a created schema object. `GET /api/2.1/unity-catalog/schemas?catalog_name=unity` confirms existing `bronze` + `gold` schemas. `DELETE` works for cleanup. The REST API is the **only** UC integration path that works with the project's Spark 4.x stack today.

**Revised Selection**: **Option A — pure-Python REST client against `/api/2.1/unity-catalog/{schemas,tables}`.**

- `scripts/bootstrap_uc_tables.py` is rewritten as a small (~200 LOC) `httpx` (or stdlib `urllib.request`) client.
- Iterates `contracts/*.yml`; skips contracts with an empty `columns:` block.
- For each populated contract:
  1. `GET /api/2.1/unity-catalog/schemas?catalog_name={catalog}` → collect existing schema names.
  2. If `contract.schema` not present: `POST /api/2.1/unity-catalog/schemas` with `{"name": schema, "catalog_name": catalog}`.
  3. `GET /api/2.1/unity-catalog/tables?catalog_name={catalog}&schema_name={schema}` → collect existing table names.
  4. If `contract.table` not present: `POST /api/2.1/unity-catalog/tables` with the full payload:
     ...
     ```json
     {
       ...
       "columns": [
         {"name": "...", "type_text": "...", "type_name": "...",
          "type_json": "{...}", "position": 0, "nullable": true}
       ],
       "storage_location": "<warehouse_root>/<env>/<layer>/<domain>/<table>/"
     }
     ```
- **No Spark. No JDBC. No UCSingleCatalog. No Maven coordinates.**
- Idempotent via GET-before-POST (replaces the implicit idempotency of `CREATE … IF NOT EXISTS`). Easier to debug — re-run produces a detailed `"X exists" / "Y created"` log per object.
- Runtime dependencies: only `httpx` (or stdlib `urllib.request`) and `pyyaml` (both already in project deps).
- Error handling: `HTTPException` → log `ERROR` with table name + status + body → accumulate failures, continue iterating, re-raise (or non-zero exit) at the end so the operator sees every failure in one pass.

**Why this is the right final answer**:
- Decouples UC registration entirely from the Spark stack. No JVM, no Scala-binary-compat dependency, no Maven artifact resolution. As UC OSS evolves its API, the script tracks the API; the user's Spark/Delta versions can evolve independently.
- The Decision 12 trade-off ("UC is UI-demo-only") is partially recovered: tables become visible in the UC UI without the Spark integration. Runtime writers stay `DeltaCatalog` + path-based (unchanged from Decision 12 + Decision 15).
- Idempotency is explicit (GET-before-POST) instead of implicit (`CREATE … IF NOT EXISTS`).

**Revised Artifact Flow (v1.18 — final)**:

```
contracts/<table>.yml  (human-edited; columns + types + nullability)
       |
       +--> scripts/bootstrap_uc_tables.py
              |  (pure-Python httpx client; no Spark, no JDBC)
              |  GET-before-POST against /api/2.1/unity-catalog/*
              v
       Unity Catalog OSS REST API (port 8080)
              |
              v  External Delta tables registered (table_type=EXTERNAL,
              |   data_source_format=DELTA, storage_location=warehouse/...)
       Runtime DAG tasks (DeltaCatalog + path-based .save) — visible in UC UI.
```

**Revised Deploy-Time Lifecycle**:
- `make dev-up` → `docker-compose up` (Postgres for Marquez, UC OSS, Airflow).
- `make bootstrap-uc` → runs `python scripts/bootstrap_uc_tables.py --env $(ENV)` (**pure-Python REST client**). No Spark, no JVM, no Liquibase invocation against UC. Idempotent via GET-before-POST.
- `_infra/cd/liquibase-apply.sh` is no longer part of the UC bring-up path; it remains available for the Postgres audit-trail use case (`JDBC_FLAVOR=postgres`, the pre-v1.16 default).

**Revised Trade-offs**:
- Bootstrap requires Python 3.12 + `httpx` + `pyyaml` on the host running `make bootstrap-uc` — already satisfied by project deps. **No JVM required** (the v1.17 trade-off goes away).
- No more `UCSingleCatalog` commit-path concerns — there is no Spark in this path at all.
- No Liquibase audit history for UC schema changes (only the `contracts/` git history). Same as v1.17. Acceptable for an OSS local-laptop demo; production deployments would add a separate Postgres-mirrored Liquibase trail if needed.
- The `uc.bootstrap_uri` default changes from `http://unity-catalog:8080` (v1.17 — only resolves inside the compose network) to `http://localhost:8080` (v1.18 — works for host invocation; override to `http://unity-catalog:8080` for in-container).

**Revised Affected Sections** (edited in this v1.18 pass):
- §2.1 — project layout tree comment + module-to-template mapping row for `scripts/bootstrap_uc_tables.py` rewritten: pure-Python REST client (no Spark).
- §2.3 — `scripts/bootstrap_uc_tables.py` row rewritten: httpx client, GET-before-POST against `/api/2.1/unity-catalog/{schemas,tables}`. Cites the Spark-4-vs-UCSingleCatalog binary-incompat block.
- §3.2 — Decision 17 deploy-time contract paragraph rewritten: `storage_location` (not SQL `LOCATION`); cite REST POST.
- §7.1 — `uc.bootstrap_uri` default changed to `http://localhost:8080`; new `uc.bootstrap_timeout_seconds` row added.
- §9.1 — "Deploy-time UC table registration (Decision 17)" paragraph rewritten per Option A (REST client; no SparkSession; no JVM).
- §9.3 — `make bootstrap-uc` row clarified to run the pure-Python REST client.
- §13 Decision 17 — this Revised v1.18 block prepended; v1.17 Spark-DDL block quarantined below; v1.16 Liquibase block remains quarantined below v1.17.
- §14 — new v1.18 row.

**Downstream Retrofits**:
- STORY-01-008 AC9 (the rewritten bootstrap_uc_tables.py AC from v1.13/v1.17 backlog): **REWRITE verifications again.** New `required_grep`: `["httpx", "schemas", "tables", "POST", "GET", "EXTERNAL", "DELTA", "storage_location", "contracts/"]`. New `forbidden_grep`: `["SparkSession", "UCSingleCatalog", "spark.sql", "JDBC", "liquibase"]`. Description: "`bootstrap_uc_tables.py` is a pure-Python REST client. No Spark, no JDBC. Idempotent via GET-before-POST."
- STORY-01-008 AC12 (config keys): **UPDATE** to include `uc_bootstrap_timeout_seconds`. The `uc_bootstrap_uri` default is now `http://localhost:8080` (was `http://unity-catalog:8080` in v1.17 — that hostname doesn't resolve outside the compose network).
- STORY-03-001: **no changes** (runtime `CREATE TABLE` prohibition still holds).

**Cross-reference**: Same root-cause family as Decision 12 (UCSingleCatalog unreliable for embedded Spark) — v1.17 underestimated that DDL-only operations hit the same Spark 4 wall. REST API smoke test verified 2026-05-23 against the running UC OSS compose service.

---

#### Revised 2026-05-23 (v1.17) — Option 4: One-Shot Spark Session with UCSingleCatalog (DDL-Only) — SUPERSEDED, preserved for audit

> **Status**: SUPERSEDED v1.18. The block below describes the v1.17 selection (one-shot SparkSession with `UCSingleCatalog` DDL-only). It is superseded by the Revised 2026-05-23 (v1.18) block above. Reason: `unitycatalog-spark_2.13:0.3.0` is built against Spark 3.5.x and binary-incompatible with the project's PySpark 4.0.x — `CREATE TABLE` raises `NoSuchMethodError` on `org.apache.spark.internal.LogKey.$init$`. No newer `unitycatalog-spark_2.13` version is published. Retained for traceability only.

**Pivot Rationale**: The v1.16 selection (Option 3 — Liquibase against UC OSS via `io.unitycatalog:unitycatalog-jdbc`) is **not executable today**. A search of `g:io.unitycatalog` on Maven Central on 2026-05-23 returns only `unitycatalog-spark_2.13`, `unitycatalog-server`, and `unitycatalog-client` (latest 0.3.0) — there is **no published `unitycatalog-jdbc` artifact**. The Databricks JDBC driver (`com.databricks:databricks-jdbc`) is mature but targets Databricks SaaS / DBSQL warehouses, not the local UC OSS REST server on port 8080. Decision 17 must pivot to a path that uses artifacts that actually exist.

**Revised Selection**: **Option 4 — one-shot Spark session with `UCSingleCatalog` (DDL-only mode).**

- `scripts/bootstrap_uc_tables.py` becomes a standalone Spark application (run via `spark-submit` or directly via `python` with PySpark on the path).
- It boots a SparkSession wired with a dedicated catalog binding `spark.sql.catalog.${UC_BOOTSTRAP_CATALOG_NAME}` (default `unity`) → `io.unitycatalog.spark.UCSingleCatalog` (the existing `unitycatalog-spark_2.13` artifact provides this). **This catalog wiring is for THIS SCRIPT ONLY** — runtime DAG tasks continue to use `DeltaCatalog` per §13 Decision 12.
- For each `contracts/<table>.yml` with a populated `columns:` list, the script issues:
  - `CREATE SCHEMA IF NOT EXISTS ${UC_BOOTSTRAP_CATALOG_NAME}.<schema>` (once per distinct schema)
  - `CREATE TABLE IF NOT EXISTS ${UC_BOOTSTRAP_CATALOG_NAME}.<schema>.<table> (<cols>) USING DELTA LOCATION '<warehouse_root>/<env>/<layer>/<domain>/<table>/'`
- Idempotent (`IF NOT EXISTS` everywhere). Re-run is safe.
- Liquibase **reverts to Postgres-only audit trail** (existing `_infra/cd/liquibase-apply.sh` behavior pre-v1.16). The `JDBC_FLAVOR=uc` mode added by v1.16 is removed.
- `gen_liquibase_from_contracts.py` is **no longer required** for UC bring-up. Disposition (delete vs repurpose as a Postgres-changelog generator for the audit trail) is **deferred** to the next `update-scaffold` pass.

**Why this works with Decision 12**: Decision 12 says `UCSingleCatalog` is "not in the runtime write path." DDL-only registration at deploy time is **not** the runtime write path — it is a one-shot bootstrap that runs before any DAG executes. The constraint that "`UCSingleCatalog` is unreliable for sustained writes in embedded Spark" does not apply to a one-shot `CREATE TABLE` script that exits immediately. Runtime tasks (Bronze/Silver/Gold writers) keep `DeltaCatalog` + path-based `.save(path)`. No change to runtime catalog wiring.
...
contracts/<table>.yml  (human-edited; columns + types + nullability)
       |
       +--> scripts/bootstrap_uc_tables.py
              |  (one-shot Spark session, UCSingleCatalog DDL-only)
              |  Reads contracts directly. Issues CREATE SCHEMA/TABLE
              |  IF NOT EXISTS for each populated contract.
              v
       Unity Catalog OSS (external Delta tables registered)
              |
              v
       Runtime DAG tasks (DeltaCatalog + path-based .save) — tables visible in UC.

(Liquibase reverts to Postgres-only audit-trail role.)
```

**Revised Deploy-Time Lifecycle**:
- `make dev-up` → `docker-compose up` (Postgres for Marquez, UC OSS, Airflow).
- `make bootstrap-uc` → runs `python scripts/bootstrap_uc_tables.py --env $(ENV)` (Spark application). **No Liquibase invocation against UC.** Idempotent.
- `_infra/cd/liquibase-apply.sh` is no longer part of the UC bring-up path; it remains available for the Postgres audit-trail use case (`JDBC_FLAVOR=postgres`, the pre-v1.16 default).

**Revised Trade-offs**:
- Bootstrap requires a Spark + JDK install on the host running `make bootstrap-uc` — **same as runtime; no new requirement**. The Docker Airflow image already has this; for host bootstrap, the existing `uv run` + PySpark dependency covers it.
- `UCSingleCatalog` has the known commit-path issues that motivated Decision 12 — but those manifest under sustained writes, not one-shot DDL. Mitigation: the bootstrap script does `CREATE … IF NOT EXISTS` only (no INSERTs, no UPDATEs, no MERGEs), single transaction per table. Fails-loud on commit error so the operator sees it immediately.
- No Liquibase audit history for UC schema changes (only the `contracts/` git history). Acceptable for an OSS local-laptop demo; production deployments would add a separate Postgres-mirrored Liquibase trail if needed.

**Revised Affected Sections** (edited in this v1.17 pass):
- §2.1 — project layout: `scripts/bootstrap_uc_tables.py` kept; `scripts/gen_liquibase_from_contracts.py` disposition deferred to next `update-scaffold`.
- §2.3 — `scripts/bootstrap_uc_tables.py` row rewritten (Spark application, not Liquibase wrapper); `scripts/gen_liquibase_from_contracts.py` row removed.
- §3.2 — Decision 17 deploy-time contract paragraph rewritten to cite the Spark application; the "warehouse path IS the UC LOCATION value" sentence kept.
- §7.1 — `uc.jdbc_url` / `uc.jdbc_driver` / `uc.jdbc_driver_jar_path` rows dropped; `uc.bootstrap_catalog_name` / `uc.bootstrap_uri` rows added.
- §9.1 — Liquibase-UC-JDBC paragraph dropped; "Deploy-time UC table registration (Decision 17)" paragraph rewritten per Option 4.
- §9.3 — `make bootstrap-uc` row clarified to run the Spark application directly.
- §13 Decision 17 — this Revised block prepended; original v1.16 body quarantined below.
- §14 — new v1.17 row.

**Downstream Retrofits**:
- STORY-01-008: AC9 (`gen_liquibase_from_contracts.py` existence) — **remove** or repurpose for Postgres audit. AC10 (`bootstrap_uc_tables.py` existence) — **keep** but rewrite verifications: now a Spark application, not a Liquibase wrapper. AC11 (`liquibase-apply.sh` UC mode) — **remove** (revert to pre-v1.16 Postgres-only). AC12 (Makefile `bootstrap-uc`) — **keep**. AC13 (canonical bring-up) — **keep**. AC14 (UC JDBC driver pinning) — **remove**.
- STORY-03-001: no changes (runtime `CREATE TABLE` prohibition still holds).

**Cross-reference**: Search of `g:io.unitycatalog` on Maven Central on 2026-05-23 returned only `unitycatalog-spark_2.13`, `unitycatalog-server`, `unitycatalog-client` (no JDBC artifact).

---

#### Original v1.16 Body (Superseded — preserved for audit)

> The block below is the **original Decision 17 selection** (Option 3 — Liquibase against UC OSS via `io.unitycatalog:unitycatalog-jdbc`). It is superseded by the Revised 2026-05-23 (v1.17) block above. All `~~struck-through~~` references describe an artifact (`io.unitycatalog:unitycatalog-jdbc`) that is not published on Maven Central. Retained for traceability only.

**Context**: Decision 12 (revised 2026-05-12) removed `UCSingleCatalog` from the runtime write path — runtime Spark uses `DeltaCatalog` + embedded Derby, and UC OSS is "UI demo only". Acknowledged consequence (Decision 12 + Decision 15 Trade-offs): Bronze/Silver/Gold tables are not discoverable in the UC UI without a manual external-table registration step. The 2026-05-22 silver run reproduced this gap end-to-end — `apply_scd2` had to direct-edit a `CREATE TABLE IF NOT EXISTS … USING DELTA LOCATION …` call into the SCD2 helper to make `clinical.clinical_patients` visible to UC queries after the first write (`chapter-6/developer-plugin/LLD-DEVIATIONS.md` row 10, to be added). The same run also exposed a cold-warehouse first-run `DELTA_MERGE_UNRESOLVED_EXPRESSION` failure that the helper did not handle.

Infrastructure already partially in place but never wired together: `contracts/*.yml` (24+ table contracts with full column lists), `ddl/liquibase/master-changelog.xml` + `changelogs/` (29 stub changelogs with metadata-only columns and the stub comment `Columns stubbed; run update-scaffold sync-contracts to populate from DMS.`), and `_infra/cd/liquibase-apply.sh` (currently targets the Postgres `marquez-db`).

**Options Considered**:
1. **Status quo** — runtime writers call `CREATE TABLE IF NOT EXISTS … USING DELTA LOCATION …` ad-hoc when they need UC visibility (today's direct-edit state). Brittle, non-deterministic, scattered across writers.
2. **`bootstrap_uc_tables.py` against UC REST API** — bespoke client, more code to own.
3. ~~**Deploy-time registration via Liquibase against UC OSS using the open-source UC JDBC driver** (`io.unitycatalog:unitycatalog-jdbc`). Contracts are the source of truth; a generator (`scripts/gen_liquibase_from_contracts.py`) replaces the stub changelogs with real column lists derived from `contracts/*.yml`. `_infra/cd/liquibase-apply.sh` re-pointed at UC JDBC. Runtime writers stay path-based via `DeltaCatalog` and are **FORBIDDEN** from calling `CREATE TABLE`.~~ **[SUPERSEDED v1.17 — `io.unitycatalog:unitycatalog-jdbc` is not published on Maven Central as of 2026-05-23.]**

**Selected (v1.16, superseded v1.17)**: ~~Option 3.~~ See Revised 2026-05-23 (v1.17) block above — replacement is **Option 4 (one-shot Spark with `UCSingleCatalog` DDL-only)**.

**Rationale**:
- Single human-edited source-of-truth (`contracts/*.yml`) — same files DQ rules already reference.
- Liquibase gives audit history (`DATABASECHANGELOG`), rollback semantics, and idempotent apply across re-runs.
- Reuses the existing scaffolding (changelogs dir, master-changelog, apply script) instead of introducing a parallel bespoke registrar.
- UC OSS is unblocked as a real catalog surface (not just a UI demo) at deploy time, without re-introducing `UCSingleCatalog` into the runtime write path — Decision 12 still holds.
- Runtime simplicity preserved: `DeltaCatalog` + `.save(<absolute-path>)`. No SQL `CREATE TABLE` inside writers, no race conditions, no `DELTA_CREATE_TABLE_WITH_NON_EMPTY_LOCATION` on re-runs after a Derby reset.
...
contracts/<table>.yml  (human-edited; columns + types + nullability)
       |
       +--> scripts/gen_liquibase_from_contracts.py  (rewrites ddl/liquibase/changelogs/<table>.xml)
       |       |
       |       +--> ddl/liquibase/master-changelog.xml (existing; references each changelog)
       |
       +--> dq_rules/<table>.yml (existing flow)

ddl/liquibase/master-changelog.xml
       |
       v
_infra/cd/liquibase-apply.sh   (env-driven: JDBC_URL, JDBC_DRIVER, drivers volume)
       |
       |  JDBC: io.unitycatalog:unitycatalog-jdbc
       |  URL : jdbc:unitycatalog://unity-catalog:8080  (compose service)
       v
Unity Catalog OSS (catalogs: clinical, reference, billing, bronze, ...)
       |
       v  (Delta external tables registered with LOCATION = warehouse/{env}/<layer>/<domain>/<table>/)
Runtime Spark writers (DeltaCatalog + path-based .save) — tables NOW visible in UC.
```

**Deploy-Time Lifecycle**:
- `make dev-up` → `docker-compose up` (Postgres for Marquez, UC OSS, Airflow) → `make bootstrap-uc` (NEW Makefile target, §9.3).
- `make bootstrap-uc` runs: (1) `python scripts/gen_liquibase_from_contracts.py` regenerates changelogs from contracts; (2) `bash _infra/cd/liquibase-apply.sh` runs `liquibase update` against UC JDBC — idempotent (Liquibase skips already-applied changesets).
- Liquibase container gets the `unitycatalog-jdbc` driver mounted via volume + `--driver` flag (see §9.1 deploy-time UC table registration paragraph).

**Runtime Lifecycle**:
- Bronze/Silver/Gold tasks: `DeltaCatalog` `spark_catalog`; `.save(<absolute-path>)` only.
- NO `CREATE TABLE` in writers; NO `MERGE INTO <fqn>` using catalog FQNs that depend on registration — use `delta.\`<path>\`` MERGE syntax when needed.
- UC visibility is a deploy-time invariant, not a runtime invariant. If UC is down at runtime, writes still succeed (Decision 12 — UC is a non-blocking dependency).

**Contract for `apply_scd2`** (revokes the 2026-05-22 direct-edit, `chapter-6/developer-plugin/LLD-DEVIATIONS.md` row 10):
- Helper MUST NOT call `CREATE TABLE` anymore.
- First-run cold-warehouse handling stays: if `_delta_log/` is absent under `target_path`, write the initial Delta version via `.format("delta").mode("overwrite").save(target_path)` — same path-based pattern, no catalog DDL.
- `target_table` keyword argument is removed from the §2.3 signature.

**Trade-offs**:
- New deploy-time dependency: `make bootstrap-uc` must run after `make dev-up` and after any contract change. Mitigation: chain `bootstrap-uc` into the `dev-up` target or document as a required step.
- Liquibase container needs the UC JDBC driver. Mitigation: extend `_infra/cd/liquibase-apply.sh` to download/mount the driver JAR (§9.1).
- Schema drift between contracts and actual Delta data is possible if a writer adds a column not in the contract. Mitigation: contract test (§2.4) asserts every Delta table's columns ⊇ contract columns.

**Affected Sections**: §2.1 (project layout — new scripts), §2.3 (`scd2.py` Responsibility — `target_table` removed, `CREATE TABLE` forbidden; new entries for `scripts/gen_liquibase_from_contracts.py` and `scripts/bootstrap_uc_tables.py`), §3.2 (storage layout — warehouse path IS the LOCATION registered with UC at deploy time), §7.1 (new `uc.jdbc_url`, `uc.jdbc_driver`, `uc.jdbc_driver_jar_path`), §9.1 (Liquibase container UC JDBC driver mount; deploy-time bootstrap paragraph), §9.3 (`make bootstrap-uc` target), §13 Decision 17 (this entry).

**Downstream Retrofits (stories pass)**:
- STORY-01-008 (runtime-bootstrap): add ACs for `scripts/gen_liquibase_from_contracts.py`, `scripts/bootstrap_uc_tables.py`, `make bootstrap-uc` target, Liquibase container UC JDBC driver wiring.
- STORY-03-001: add AC explicitly forbidding `CREATE TABLE` in `transform_patients.py` and `apply_scd2`.
...
### Decision 16 (candidate, 2026-05-21): Migrate Hive Metastore to Postgres for DEV Concurrency > 1

**Status**: Candidate — not yet number-assigned formally. Will be the next sequential decision when the team accepts it. Listed here as a placeholder so future readers see the trade-off that surfaced on 2026-05-21.

**Context**: §13 Decision 12 selects an embedded Derby Hive metastore for DEV. Derby is a single-process database; only one JVM may open the metastore at a time. As a result, §4.1 caps DEV `max_active_tasks=1`, serializing the 13 Bronze ingest tasks. The 2026-05-21 attempt to raise concurrency to 16 reproduced `ERROR XSDB6: Another instance of Derby may have already booted the database null` on the first DAG run.

**Pending scope**:
1. Cost of migration to a multi-client metastore backend.
2. Container choice — Hive Metastore Standalone vs reuse Marquez's existing Postgres instance (already present in the compose stack as `marquez-db`).
3. Validation against UC OSS replay (Decision 12 retains UC OSS as a UI-demo surface; the new metastore must not break that path).
4. Whether the educational goal of chapter-5 (zero-config local stack) is preserved if Postgres replaces Derby.

**Not yet selected.** Open for team discussion. When accepted, this decision will be formally numbered and §4.1 DEV `max_active_tasks` revisited.
...
|---------|------|--------|---------|
| 1.18 | 2026-05-23 | Technical Lead Agent | Status changed to Approved |
| 1.18 | 2026-05-23 | Technical Lead Agent | **SECOND PIVOT of Decision 17 — Spark-DDL path (v1.17) abandoned because `unitycatalog-spark_2.13:0.3.0` is binary-incompatible with Spark 4.x** (`java.lang.NoSuchMethodError: 'void org.apache.spark.internal.LogKey.$init$(org.apache.spark.internal.LogKey)'`). Reproduced 2026-05-23 with `make bootstrap-uc` — `UCProxy` (delta 4.0 ScalaModule) triggered the failure on `CREATE TABLE IF NOT EXISTS unity.silver.clinical_patients USING DELTA LOCATION '...'`. Root cause: latest published `unitycatalog-spark_2.13` artifact (0.3.0, 2025-06-06) targets Spark 3.5.x; the `org.apache.spark.internal.LogKey` trait was restructured between Spark 3.5 and Spark 4.0; UCSingleCatalog calls a method that no longer exists. No newer `unitycatalog-spark_2.13` version published (0.2.0, 0.2.1, 0.3.0 — all pre-Spark-4). Same root-cause family as Decision 12 (UCSingleCatalog unreliable for embedded Spark) — v1.17 underestimated that even DDL-only operations hit the same Spark 4 wall. **Replaced with Option A — pure-Python REST client against `/api/2.1/unity-catalog/{schemas,tables}` endpoints** (verified working via `curl` smoke test 2026-05-23: `POST /schemas` creates, `GET /schemas` lists, `DELETE` cleans up). `scripts/bootstrap_uc_tables.py` rewritten as a ~200 LOC `httpx` (or stdlib `urllib.request`) client. Iterates `contracts/*.yml`; skips empty `columns:` blocks. Per populated contract: (1) GET existing schemas → POST if missing; (2) GET existing tables → POST if missing with full payload (`table_type="EXTERNAL"`, `data_source_format="DELTA"`, `columns` list, `storage_location="<warehouse_root>/<env>/<layer>/<domain>/<table>/"`). **No SparkSession. No JDBC. No UCSingleCatalog. No Maven coordinates.** Idempotent via GET-before-POST (replaces implicit `CREATE … IF NOT EXISTS` idempotency). Error handling: HTTPException → log ERROR with table name + status + body → accumulate failures, continue iterating, re-raise at end (non-zero exit). **Edits applied**: (1) §2.1 — project layout tree comment + module-to-template mapping row for `scripts/bootstrap_uc_tables.py` rewritten as pure-Python REST client (no Spark, no JDBC, no UCSingleCatalog). (2) §2.3 — `scripts/bootstrap_uc_tables.py` module section rewritten: httpx client, GET-before-POST against `/api/2.1/unity-catalog/{schemas,tables}`, fail-loud-at-end error handling, no JVM required. Cites the Spark-4-vs-UCSingleCatalog binary-incompat reproduction. (3) §3.2 — Decision 17 deploy-time contract paragraph rewritten: warehouse path IS the `storage_location` value POSTed to UC (not the SQL `LOCATION` clause); cite REST POST to `/api/2.1/unity-catalog/tables`. (4) §7.1 — `uc.bootstrap_uri` default changed from `http://unity-catalog:8080` to `http://localhost:8080` (v1.17 default failed for host bootstrap with `UnresolvedAddressException`; only resolves inside the compose network); description clarified (host vs in-container); new `uc.bootstrap_timeout_seconds` (default 30, env var `UC_BOOTSTRAP_TIMEOUT_SECONDS`) row added; `uc.bootstrap_catalog_name` description rewritten (no more `UCSingleCatalog` binding — just the `catalog_name` query param/payload field). (5) §9.1 — `ddl/liquibase/` row updated to cite v1.18 pure-Python REST client as UC registration owner; "Deploy-time UC table registration (Decision 17)" paragraph rewritten per Option A (REST client; no SparkSession; no JVM; cites both the Spark-4 binary-incompat that killed v1.17 and the curl smoke that validated v1.18). (6) §9.3 — `make bootstrap-uc` row rewritten: pure-Python httpx client invocation; GET-before-POST; no Spark, no JDBC, no JVM. (7) §13 Decision 17 — Status updated to "SECOND PIVOT"; new "Revised 2026-05-23 (v1.18) — Option A: Pure-Python REST Client (ACTIVE)" block prepended (full Pivot Rationale with the LogKey stack trace, Revised Selection enumerating the 4 GET/POST steps + JSON payload shape, Why-this-is-the-right-final-answer, Revised Artifact Flow diagram, Revised Deploy-Time Lifecycle, Revised Trade-offs including the `uc.bootstrap_uri` default change, Revised Affected Sections, Downstream Retrofits, Cross-reference); v1.17 Spark-DDL block headed with "SUPERSEDED, preserved for audit" callout; v1.16 Liquibase-UC-JDBC block remains preserved unchanged at the bottom. Decision 17 now carries three stacked blocks (newest first): v1.18 REST (active) → v1.17 Spark-DDL (audit) → v1.16 Liquibase UC JDBC (audit). **Downstream retrofits**: STORY-01-008 AC9 — REWRITE verifications again. New `required_grep`: `["httpx", "schemas", "tables", "POST", "GET", "EXTERNAL", "DELTA", "storage_location", "contracts/"]`. New `forbidden_grep`: `["SparkSession", "UCSingleCatalog", "spark.sql", "JDBC", "liquibase"]`. STORY-01-008 AC12 — UPDATE to include `uc_bootstrap_timeout_seconds`; `uc_bootstrap_uri` default is now `http://localhost:8080` (was `http://unity-catalog:8080` in v1.17). STORY-03-001 — no changes (runtime `CREATE TABLE` prohibition still holds). Scenario C update: v1.17 → v1.18 in-place (same date 2026-05-23). Status reset to `Updated - Pending Review` — re-approval required before downstream stories re-cut. |
| 1.17 | 2026-05-23 | Technical Lead Agent | Status changed to Approved |
| 1.17 | 2026-05-23 | Technical Lead Agent | **PIVOTED Decision 17 — Liquibase-against-UC-JDBC path (v1.16) abandoned because `io.unitycatalog:unitycatalog-jdbc` is not published on Maven Central as of 2026-05-23.** Search of `g:io.unitycatalog` on Maven Central returned only `unitycatalog-spark_2.13`, `unitycatalog-server`, `unitycatalog-client` (latest 0.3.0) — no JDBC artifact. The v1.16 plan was unbuildable. **Replaced with Option 4 — one-shot Spark session with `UCSingleCatalog` (DDL-only)**. `scripts/bootstrap_uc_tables.py` becomes a standalone Spark application: boots a SparkSession with `spark.sql.catalog.${UC_BOOTSTRAP_CATALOG_NAME}` → `io.unitycatalog.spark.UCSingleCatalog` (from the existing `unitycatalog-spark_2.13` artifact; distinct from runtime `spark_catalog` which stays `DeltaCatalog` per Decision 12), reads `contracts/*.yml` directly, issues `CREATE SCHEMA IF NOT EXISTS` + `CREATE TABLE IF NOT EXISTS … USING DELTA LOCATION '<warehouse_root>/<env>/<layer>/<domain>/<table>/'` per populated contract. Idempotent (`IF NOT EXISTS` everywhere). UCSingleCatalog's sustained-write commit-path issues (the reason Decision 12 demoted it from the runtime write path) do not apply — this is one-shot DDL only (no INSERTs, no UPDATEs, no MERGEs). Liquibase reverts to its pre-v1.16 Postgres-only audit-trail role; the `JDBC_FLAVOR=uc` mode added by v1.16 in `_infra/cd/liquibase-apply.sh` is removed. `scripts/gen_liquibase_from_contracts.py` is no longer required for UC bring-up; its disposition (delete vs repurpose for Postgres audit) is deferred to the next `update-scaffold` pass. **Edits applied**: (1) §2.3 — `scripts/bootstrap_uc_tables.py` row rewritten (Spark application; UCSingleCatalog binding for THIS SCRIPT ONLY; runtime tasks unchanged); `scripts/gen_liquibase_from_contracts.py` row removed. (2) §3.2 — Decision 17 deploy-time contract paragraph rewritten to cite the Spark application instead of Liquibase. (3) §7.1 — `uc.jdbc_url` / `uc.jdbc_driver` / `uc.jdbc_driver_jar_path` rows dropped; `uc.bootstrap_catalog_name` (default `unity`, env var `UC_BOOTSTRAP_CATALOG_NAME`) and `uc.bootstrap_uri` (default `http://unity-catalog:8080`, env var `UC_BOOTSTRAP_URI`) rows added. (4) §9.1 — `ddl/liquibase/` row rewritten (Postgres-only audit role); the "Deploy-time UC table registration (Decision 17)" paragraph rewritten per Option 4 (Spark application; no Liquibase invocation against UC). (5) §9.3 — `make bootstrap-uc` row rewritten to run `python scripts/bootstrap_uc_tables.py --env $(ENV)` directly. (6) §13 Decision 17 — Revised 2026-05-23 (v1.17) block prepended (full Pivot Rationale / Revised Selection / Why-this-works-with-Decision-12 / Revised Artifact Flow / Revised Deploy-Time Lifecycle / Revised Trade-offs / Revised Affected Sections / Downstream Retrofits / Cross-reference); original v1.16 Option 3 selection line struck-through with `[SUPERSEDED v1.17]` marker; the rest of the original v1.16 body is preserved underneath the new "Original v1.16 Body (Superseded — preserved for audit)" header for traceability. **Downstream retrofits**: STORY-01-008 — AC9 (`gen_liquibase_from_contracts.py` existence) remove or repurpose for Postgres audit; AC10 (`bootstrap_uc_tables.py` existence) keep but rewrite verifications (now a Spark application); AC11 (`liquibase-apply.sh` UC mode) remove (revert to pre-v1.16 Postgres-only); AC12 (Makefile `bootstrap-uc`) keep; AC13 (canonical bring-up) keep; AC14 (UC JDBC driver pinning) remove. STORY-03-001 unchanged (runtime `CREATE TABLE` prohibition still holds). Scenario C update: v1.16 → v1.17 in-place (same date 2026-05-23). Status reset to `Updated - Pending Review` — re-approval required before downstream stories re-cut. |
| 1.16 | 2026-05-23 | Technical Lead Agent | Status changed to Approved |
| 1.16 | 2026-05-23 | Technical Lead Agent | **NEW Decision 17 — Contracts-as-Source + Deploy-Time UC Table Registration via Liquibase (UC JDBC).** Closes the long-standing "Delta files invisible in UC" gap acknowledged as a Trade-off in Decisions 12 + 15. Background: 2026-05-22 silver run reproduced the gap end-to-end (`apply_scd2` was direct-edited to call `CREATE TABLE IF NOT EXISTS … USING DELTA LOCATION …` to make `clinical.clinical_patients` visible — see `chapter-6/developer-plugin/LLD-DEVIATIONS.md` row 10) and also exposed a cold-warehouse first-run `DELTA_MERGE_UNRESOLVED_EXPRESSION` failure. Resolution: contracts are the source of truth; a new generator (`scripts/gen_liquibase_from_contracts.py`) replaces the stub changelogs in `ddl/liquibase/changelogs/*.xml` with real column lists derived from `contracts/*.yml`; `_infra/cd/liquibase-apply.sh` is re-pointed at UC OSS via the open-source UC JDBC driver (`io.unitycatalog:unitycatalog-jdbc`); a new `make bootstrap-uc` target chains generator + apply and is idempotent. Runtime writers stay path-based via `DeltaCatalog` and are **FORBIDDEN** from calling `CREATE TABLE` — UC visibility is a deploy-time invariant, not a runtime invariant. Decision 12 still holds (UC is non-blocking at runtime). **Edits applied**: (1) §2.1 — added `scripts/gen_liquibase_from_contracts.py` and `scripts/bootstrap_uc_tables.py` to the project layout tree and the module-to-template mapping table. (2) §2.3 — `scd2.py` Responsibility rewritten: `target_table` kwarg removed; `CREATE TABLE` explicitly forbidden (revokes the 2026-05-22 direct-edit); cold-warehouse first-run handling stays path-based (`format("delta").mode("overwrite").save(target_path)` when `_delta_log/` is absent); use `delta.\`<path>\`` MERGE syntax when needed. Added two new module entries — `scripts/gen_liquibase_from_contracts.py` (Input: `contracts/*.yml`; Output: `ddl/liquibase/changelogs/*.xml`) and `scripts/bootstrap_uc_tables.py` (wraps `_infra/cd/liquibase-apply.sh` with `JDBC_URL` pointing at UC OSS). (3) §3.2 — added Decision 17 deploy-time contract paragraph: every `warehouse/{env}/<layer>/<domain>/<table>/` path IS the `LOCATION` value registered with UC at deploy time. (4) §7.1 — added three new catalog parameters (`uc.jdbc_url`, `uc.jdbc_driver`, `uc.jdbc_driver_jar_path`) consumed by `_infra/cd/liquibase-apply.sh`. (5) §9.1 — Liquibase container needs `unitycatalog-jdbc` driver mounted via volume + `--driver` flag; added a "Deploy-time UC table registration (Decision 17)" paragraph spelling out the `make bootstrap-uc` lifecycle and the runtime `CREATE TABLE` prohibition. (6) §9.3 — added `make bootstrap-uc` to the Local Make Targets table. (7) §13 — added full Decision 17 entry with Options/Selected/Rationale/Artifact-Flow/Deploy-Time-Lifecycle/Runtime-Lifecycle/`apply_scd2`-Contract/Trade-offs/Affected-Sections/Downstream-Retrofits. **Downstream impact**: STORY-01-008 (runtime-bootstrap) gains ACs for the two new scripts + Make target + Liquibase driver wiring; STORY-03-001 gains AC forbidding `CREATE TABLE` in `transform_patients.py` and `apply_scd2`. Cross-references `chapter-6/developer-plugin/LLD-DEVIATIONS.md` row 10 (`apply_scd2 CREATE TABLE direct-edit`, 2026-05-22) and `chapter-6/memory/developer/learnings-queue.jsonl` entries for silver path/DDL handling. Scenario B update: v1.15 → v1.16, new filename `LLD-2026-05-23-patient-360.md`, prior file archived to `LLD-2026-05-21-patient-360.md.bak`. Status reset to `Updated - Pending Review` — re-approval required before downstream stories re-cut. |
| 1.15 | 2026-05-21 | Technical Lead Agent | Status changed to Approved |
| 1.15 | 2026-05-21 | Technical Lead Agent | **DEV `max_active_tasks` reverted 16 → 1; §13 Decision 12 Constraints paragraph added; §13 Decision 16 candidate opened.** Resolves a contradiction between §13 Decision 12 (revised 2026-05-12 — runtime stack is `DeltaCatalog` + embedded Derby Hive metastore) and the 2026-05-20 v1.14 raise of DEV `max_active_tasks` to 16. Reproduced 2026-05-21: with `max_active_tasks=16`, parallel `SparkSubmitOperator` tasks fail with `ERROR XSDB6: Another instance of Derby may have already booted the database null` on the first DAG run. Root cause: embedded Derby is a single-process database — only one JVM at a time may open the metastore. The `bootstrap_se` task added in v1.14 only solves the SE-table CREATE race; it does NOT bypass Derby's concurrent-access limit. The v1.14 raise was based on an incomplete read of the bottleneck. **Edits applied**: (1) §13 Decision 12 — appended a "Constraints (added 2026-05-21)" paragraph stating the Derby single-process limit and the `max_active_tasks=1` cap requirement in DEV; cited the XSDB6 reproduction; expanded Affected Sections to include §4.1 and the Decision 16 candidate. (2) §4.1 — reverted DEV `max_active_tasks` from 16 to 1 (and `Concurrency` to match); rationale cell rewritten to cite Decision 12 Constraints and the XSDB6 reproduction; explicit note that the 2026-05-20 raise was based on an incomplete bottleneck read. STAGING/PROD unchanged (8 / 16) — those environments are out of scope for Derby. (3) §4.2 — explanatory note under `bootstrap_se` rationale extended with a "Note (2026-05-21 correction)" paragraph clarifying that `bootstrap_se` + `max_active_tasks=1` together close BOTH failure modes (SE-table CREATE race + Derby concurrency); also updated the Bronze TaskGroup prose to say "in DEV the 13 tasks serialize behind the Derby single-process cap" instead of fanning out. No topology change — `bootstrap_se` task entry and the §4.3 DAG diagram are preserved. (4) §13 — added Decision 16 candidate placeholder (Postgres metastore migration for DEV concurrency > 1); status candidate, not number-assigned yet; pending scope items listed (cost, container choice — HMS standalone vs reuse `marquez-db` Postgres — and UC OSS replay validation). (5) §14 — this new v1.15 row. Change Log lineage: references `chapter-6/developer-plugin/LLD-DEVIATIONS.md` row 6 (incomplete prior fix) and the 2026-05-21 XSDB6 reproduction. Scenario B update: v1.14 → v1.15, new filename `LLD-2026-05-21-patient-360.md`, prior file archived to `LLD-2026-05-20-patient-360.md.bak`. Status reset to `Updated - Pending Review` — re-approval required before downstream stories re-cut. |
| 1.14 | 2026-05-20 | Technical Lead Agent | **Added `bootstrap_se` root task (§4.2).** New SparkSubmitOperator runs at the head of every DAG run, upstream of the entire `bronze_ingestion` TaskGroup. Executes `scripts/bootstrap_se_tables.py` (resolved via `SE_BOOTSTRAP_APP` env var; default `/opt/patient_360/scripts/bootstrap_se_tables.py`) under `spark_default` with 1g driver/1g executor/1 core/1 executor, `retries=1`, `retry_delay=60s`, `execution_timeout=5m`. Idempotent: `CREATE SCHEMA IF NOT EXISTS bronze` + `CREATE TABLE IF NOT EXISTS bronze.bronze_se_stats USING DELTA LOCATION '...'` (and matching `bronze_se_errors`); no-op when both tables already exist. **Rationale**: closes the SE shared-stats-table CREATE-race on cold-start metastores (`max_active_tasks > 1` produced intermittent Bronze failures) and the warm-restart `DELTA_CREATE_TABLE_WITH_NON_EMPTY_LOCATION` failure on on-disk Delta data after a container/metastore reset (encountered 2026-05-20; `chapter-6/developer-plugin/LLD-DEVIATIONS.md` row 6; `chapter-6/memory/developer/learnings-queue.jsonl`). **§4.1 concurrency restored**: with `bootstrap_se` in place, `max_active_tasks` reverts to the originally specified value (DEV: 16, STAGING: 8, PROD: 16) — the prior DEV workaround of `max_active_tasks=1` is lifted. All 13 Bronze ingest rows updated from `Dependencies: None` to `Dependencies: bootstrap_se`. §4.3 DAG diagram updated to render `BS[bootstrap_se] --> B01..B13`. §13 decisions untouched (existing decisions remain valid). Scenario B update: v1.13 → v1.14, new filename `LLD-2026-05-20-patient-360.md`, prior file archived to `LLD-2026-05-19-patient-360.md.bak`. Status reset to `Updated - Pending Review`. |
...
| 1.13 | 2026-05-20 | Technical Lead Agent | Status changed to Approved |
| 1.13 | 2026-05-19 | Technical Lead Agent | **Silver/Gold readiness — 3 deviations from chapter-6/silver-gold-plugin/LLD-DEVIATIONS.md folded in.** **(1)** §2.3 `scd2.py` — signature corrected to `apply_scd2(df, target_path, natural_keys, hash_columns, effective_date) -> dict[str, int]`. The prior signature lacked `target_path`, leaving the function unable to locate the Delta table to MERGE INTO. Output formalized as `{"rows_inserted", "rows_closed", "rows_unchanged"}` metrics dict. Spark session derived internally via `df.sparkSession` (not a parameter). Explicit prohibition on `monotonically_increasing_id()` for surrogate-key generation (non-deterministic per executor and per re-run; breaks SCD2 history); deterministic alternatives `xxhash64(natural_key, effective_date)` or `max(surrogate_key) + row_number()` mandated. DQ contract reaffirmed: caller MUST run `se_runner.run_dq` BEFORE calling `apply_scd2`; the helper trusts every row it receives. **(2)** §5.2 "DQ Check" column — added a clarifying note that the cell encodes TWO independent gates (`empty_input_behavior` ∈ {fail, write_empty} evaluated BEFORE transform; `se_action_if_failed` ∈ {fail, drop, ignore} evaluated AFTER transform, BEFORE write). Critical-table policy spelled out. Per-rule `action_if_failed` declarations still take precedence over the fail-closed default. **(3)** Implicit alignment with chapter-5 developer-plugin learnings (inherited into `chapter-6/silver-gold-plugin/skills/create-silver/SKILL.md` Inherited Learnings IL-001..IL-017): pyspark pin floor, `DeltaCatalog` over `UCSingleCatalog`, persistent Derby `ConnectionURL`, 2-part FQNs, `PATIENT360_PROJECT_ROOT` anchoring, SparkSubmitOperator for every Spark-touching task, `DagContext.get_current()` for TaskGroup factories, SE `with_expectations` lambda wrapping, SE stats table mandatory, reconciliation filter on `meta_dq_run_date`. These are already documented elsewhere in this LLD; the IL-NNN block in the chapter-6 skill cites them so Silver/Gold code generators inherit the rules verbatim. Scenario B update: v1.12 → v1.13, new filename `LLD-2026-05-19-patient-360.md`, prior file archived to `LLD-2026-05-12-patient-360.md.bak`. Status reset to `Updated - Pending Review` — re-approval required before chapter-6 Silver/Gold story dispatch. |
...
| 1.14 | 2026-05-20 | Technical Lead Agent | Status changed to Approved |
