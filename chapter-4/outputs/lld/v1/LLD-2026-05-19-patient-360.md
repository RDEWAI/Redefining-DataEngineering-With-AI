...
- **Input**: `df: DataFrame` (incoming, DQ-validated records — caller MUST have already invoked `se_runner.run_dq` before calling this function), `target_path: str` (Delta table path, e.g. `warehouse/{env}/silver/clinical/clinical_patients/`), `natural_keys: list[str]`, `hash_columns: list[str]`, `effective_date: date`
...
**Affected sections**: §1 (Design Overview — UC OSS now "UI demo only"), §2.3 (Bronze runner contract reverts to path-based Delta writes against `warehouse/{env}/bronze/{table}/`), §3.1 (storage format — drop "registered in UC"), §5.1 (Output column reverts to `warehouse/{env}/bronze/{table}/`), §7 (config schema — catalog block redirected to Hive metastore parameters), §8.1 (circuit breaker — UC OSS becomes a non-blocking dependency like Marquez/Grafana), §9.5 (health checks — UC health degraded to WARNING), §13 Decision 15 (revoked; see Decision 15 update).
