...
- **Input**: DataFrame, table name (string; SE runner resolves rules from `dq_rules/{table}.yml`), action_if_failed config
- **Output**: Validated DataFrame (rows passing DQ), quarantine DataFrame (rows failing DQ), pass/fail metrics
- **Responsibility**: Wraps spark-expectations for inline SE validation. Loads per-table YAML rules from `dq_rules/{table}.yml` (convention-based), executes row_dq and agg_dq checks with configurable action_if_failed (fail/drop/ignore), routes rejections to quarantine paths, emits pass/fail metrics. For rule definitions, see DQS §2-4.
