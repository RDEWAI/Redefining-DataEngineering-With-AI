...
- **Input**: Per-table YAML config path (resolved via `AIRFLOW_CONFIGS_DIR` env, default `/opt/airflow/configs`), Spark session built with `UCSingleCatalog` (see §13 Decision 12), pipeline run context
- **Output**: Delta table managed by Unity Catalog at `unity.bronze.<table>` (NOT path-based). UC sees the table at write time — no manual `docker cp` or external-table registration needed. Files land under `<UC warehouse>/bronze/<table>/ds=<ds>/` but the LLD does not depend on that path.
- **Responsibility**: Reads YAML config to determine source table, StructType schema, DQ rule references (resolved via `DQ_RULES_DIR` env, defaulting to `/opt/<project>/dq_rules`, then `dq_rules/{table}.yml` by table name convention), and empty-input behavior. Executes the standard ingestion pattern: read source, add metadata columns, enforce StructType schema (no inference), call SE inline for row_dq/agg_dq validation, write via `df.write.mode("append").format("delta").partitionBy("ds").option("replaceWhere", "ds = '<ds>'").saveAsTable(f"unity.bronze.<table>")`. For schema definitions, see DMS §2. **NEVER use `df.write.format("delta").save(<path>)` for Bronze** — the validator (`validate-dag UC-WIRING-001`) rejects path-based Bronze writes.
...
- **Responsibility**: Wraps spark-expectations for inline SE validation. Loads per-table YAML rules from `dq_rules/{table}.yml` (convention-based discovery using `dq_rules_dir / f"{table}.yml"`), executes row_dq and agg_dq checks with configurable action_if_failed (fail/drop/ignore), routes rejections to `quarantine_path` (from per-table YAML config in §7), emits pass/fail metrics. For rule definitions, see DQS §2-4.
  ...
  **`dq_env` mapping** (`env` parameter → SE `dq_env`):
  ...
  > **TEMP — bootstrap phase only (Decision 14, §8.6):** the `[PENDING IMPLEMENTATION]` marker on this module covers a temporal lifecycle, not a permanent contract. **Read §8.6 + Decision 14 together — §8.6 is the single source of truth for the SE Bootstrap Mode Degradation lifecycle. Do not author ACs from §2.3 alone.** Bootstrap-mode behavior (soft-import + WARNING log) is the *transitional* state; the *steady* state is fail-closed. Stories that decompose this lifecycle MUST treat the two phases as an ordered pair (bootstrap story first, fail-closed story `Depends-On` it) — see scrum-master `STORIES-AC-CONTRADICTION-001` for the validator that enforces this. Both phases are described in §8.6 only; §2.3 deliberately avoids duplicating the lifecycle prose so contradictory ACs cannot be generated.
...
| SE bootstrap mode active in STAGING/PROD | Bootstrap (§8.6) | CRITICAL | PagerDuty `p360-critical` | 15 min (fail-closed post-implementation) |

### 8.6 SE Bootstrap Mode Degradation
...
**Action required**: Once `se_runner.py` is shipped, the soft-import try/except block MUST be removed from `ingestion_runner.py`. Ingestion must fail closed if SE is unavailable — a missing SE module in post-implementation is a deployment error, not a graceful degradation. See §13 Decision 14 for the STAGING promotion checklist.
...
|----------|----------|----------|--------------|
| `bronze_se_stats` row keyed by `meta_dq_run_id` matching the run's `--ds` | SE `se.enable.stats.table=true` (auto-written by `with_expectations`) | reconciliation_bronze | `reconciliation_bronze` queries this table at end of layer; if no row exists for the current `meta_dq_run_id`, the task fails-closed with `SE_RUN_MISSING_FOR_DS=<ds>` |
...
```sql
SELECT count(*) FROM unity.bronze.bronze_se_stats
 WHERE meta_dq_run_id = '${run_id}' AND meta_dq_run_date = '${ds}'
```

and fail-closed if the count is 0. This eliminates the silent-DQ failure mode. The bootstrap story's `## How to Test (User)` block exercises this gate end-to-end (`make smoke-se` or pytest equivalent) so a human reviewer SEES `bronze_se_stats >= 1 row` before approving Done. The scrum-master rules `STORIES-SE-COVERAGE-001` and `STORIES-INTEGRATION-SE-001` enforce this requirement at backlog generation time; a backlog without the SE run-evidence ACs is rejected before the developer-plugin reads it.
...
### Decision 15: Bronze Lands in Unity Catalog at Write Time (UCSingleCatalog + saveAsTable)

**Context**: A prior version of §2.3 specified path-based Delta writes for Bronze (`warehouse/{env}/bronze/{table}/ds={ds}/`) on the assumption that UC wiring would be reserved for Silver/Gold. When the Bronze pipeline ran end-to-end on the spokane sibling branch (`run_v8_162157`, 16/16 tasks green), the resulting Bronze tables were invisible in Unity Catalog OSS until a manual `docker cp` of the Delta dirs into the UC data volume + 13 external-table REST registrations. Every subsequent DAG run repeats the gap because new Delta files land in `/tmp/uc-warehouse/` inside the airflow container and UC never sees them.

**Options Considered**:
1. Keep path-based writes; add a post-hoc `register_bronze_uc` Airflow task that registers each Delta dir as an external UC table after the ingest TaskGroup completes.
2. Switch Bronze to UC-managed writes via `UCSingleCatalog` + `saveAsTable("unity.bronze.<table>")` — same wiring Silver/Gold already use.
3. Leave the gap and document the manual UC sync step in the runbook.

**Selected**: Option 2 (UC-managed writes for Bronze).

**Rationale**: Option 2 eliminates the recurring registration drift, removes the hardcoded `/tmp/uc-warehouse/` assumption from §2.3, and makes the Bronze layer first-class in UC for downstream Silver consumers. Option 1 doubles the Bronze surface (write + register) and leaks UC-specific knowledge into the orchestration layer; Option 3 is the gap we're fixing.

**Trade-off**: Bronze runners now require a UC-wired Spark session (one extra `spark.sql.catalog.unity` config block + `UC_URI` env var resolved from docker-compose). The catalog wiring is identical to what Silver/Gold use, so the pattern is consistent. `validate-dag UC-WIRING-001` rejects path-based Bronze writes (`.save("/tmp/...")`, `.save("file://...")`) at code-generation time. Reference snippet at `inputs/code/v1/scripts/ingestion_runner.py.snippet`.

**Affected sections**: §2.3 (Bronze runner contract), §5.1 (Bronze task table — output column changed from `warehouse/...` to `unity.bronze.<table>`), §7 (config schema — new `catalog.bronze_catalog_name`, `catalog.bronze_schema`, `catalog.uc_uri`), §11 (UC reference upgraded to "Bronze + Silver + Gold all UC-managed").
