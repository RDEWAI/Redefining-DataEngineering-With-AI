...
class TestReadOnlyQueriesPass:
    ...
    def test_read_only_underscore_variant(self):
        result = _run_hook(
            _bash_input(
                'duckdb data/duckdb/raw.db -read_only -c "SELECT 1;"'
            )
        )
        assert result.returncode == 0
        assert result.stdout.strip() == ""
...
class TestMissingReadonlyFlagBlocked:
    ...
    def test_duckdb_without_readonly(self):
        result = _run_hook(
            _bash_input(
                'duckdb data/duckdb/raw.db -c "SELECT COUNT(*) FROM synthea.patients;"'
            )
        )
        assert result.returncode == 0
...
class TestWriteOperationsBlocked:
    ...
    def test_delete_blocked(self):
        result = _run_hook(
            _bash_input(
                'duckdb data/duckdb/raw.db -readonly -c "DELETE FROM synthea.patients;"'
            )
        )
        assert result.returncode == 0
        output = json.loads(result.stdout)
    ...
    def test_drop_blocked(self):
        result = _run_hook(
            _bash_input(
                'duckdb data/duckdb/raw.db -readonly -c "DROP TABLE synthea.patients;"'
            )
        )
        assert result.returncode == 0
        output = json.loads(result.stdout)
    ...
    def test_alter_blocked(self):
        result = _run_hook(
            _bash_input(
                'duckdb data/duckdb/raw.db -readonly -c "ALTER TABLE synthea.patients ADD COLUMN x INT;"'
            )
        )
        assert result.returncode == 0
        output = json.loads(result.stdout)
    ...
    def test_create_blocked(self):
        result = _run_hook(
            _bash_input(
                'duckdb data/duckdb/raw.db -readonly -c "CREATE TABLE test (id INT);"'
            )
        )
        assert result.returncode == 0
        output = json.loads(result.stdout)
    ...
    def test_truncate_blocked(self):
        result = _run_hook(
            _bash_input(
                'duckdb data/duckdb/raw.db -readonly -c "TRUNCATE TABLE synthea.patients;"'
            )
        )
        assert result.returncode == 0
        output = json.loads(result.stdout)
...
class TestEdgeCases:
    ...
    def test_dot_db_in_path_triggers_inspection(self):
        ...
        result = _run_hook(
            _bash_input(
                'sqlite3 other.db "INSERT INTO test VALUES (1);"'
            )
        )
        assert result.returncode == 0
        output = json.loads(result.stdout)
