...
| `table_name` | string | Fully-qualified `catalog.schema.table` (3-part), e.g. `unity.bronze.synthea_patients`. On the Unity Catalog stack the catalog is always `unity` and the env is NOT encoded in the name (one catalog, env = warehouse path), so `table_name` is the SAME across DEV/QA/PROD. It is the SE rule-filter key AND the base for the managed `<table_name>_error`/`_stats` audit-table FQNs — a bare name crashes the UC error-table write with an empty-namespace `fullTableNameForApi` AIOOBE. |
...
```yaml
# table_name is the fully-qualified `unity.<schema>.<table>` (3-part) and is
# IDENTICAL across DEV/QA/PROD on the Unity Catalog stack — only action_if_failed
# and thresholds vary per env. Never env-prefix the name (no dev_/qa_).
dq_env:
  DEV:
    table_name: unity.bronze.patients
