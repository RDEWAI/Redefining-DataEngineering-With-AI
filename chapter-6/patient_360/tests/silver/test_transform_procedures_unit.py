"""Unit tests for silver/transform_procedures.py (cleansed fact).
...
from patient_360.silver import transform_procedures as T_MOD  # noqa: E402
...
def spark():
    ...
    builder = (
        ...
        .appName("test_transform_procedures")
    ...
    base.update(
        {
            "PATIENT": " pat-001 ",
            "ENCOUNTER": " enc-001 ",
            "START": dt.datetime(2026, 5, 30, 9, 0, 0),
            "STOP": dt.datetime(2026, 5, 30, 9, 30, 0),
            "SYSTEM": " SNOMED-CT ",
            ...
            "_source_batch_id": "synthea_procedures:2026-06-01",
        }
    )
...
def _bronze_df(spark, rows):
    ...
    type_map = {
        "START": T.TimestampType(),
        "STOP": T.TimestampType(),
...
class TestCleanse:
    ...
    def test_stm_transformations(self, spark):
        ...
        assert r["snomed_code"] == "428191000124101"  # CAST(CODE AS STRING)
        assert r["procedure_description"] == "Documentation of current medications"
        assert r["reason_description"] == "Viral sinusitis"  # TRIM
        assert str(r["base_cost"]) == "431.40"  # CAST DECIMAL(12,2)
