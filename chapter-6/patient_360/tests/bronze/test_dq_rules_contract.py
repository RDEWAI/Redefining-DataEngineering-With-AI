...
def _rule_files() -> list[Path]:
    # Scope to the 13 Bronze tables only (synthea_<stem>.yml). The
    # dq_rules/ directory also holds Silver/Gold rule YAMLs (clinical_*,
    # reference_*, billing_claims, patient_*); those are out of scope for
    # this Bronze contract and must not be swept into the parametrize set.
    return [DQ_RULES_DIR / f"{name}.yml" for name in EXPECTED_TABLE_NAMES]
...
def test_ac1_filename_stems_match_bronze_table_names() -> None:
    ...
    assert not missing and not extra, (
        f"Rule stems do not match LLD §5.1. missing={sorted(missing)} extra={sorted(extra)}"
    )
...
def test_each_rule_has_required_keys(path: Path) -> None:
    ...
    for rule in rules:
        ...
        assert not missing, f"{path.name}: rule {rule.get('rule')!r} missing keys {missing}"
