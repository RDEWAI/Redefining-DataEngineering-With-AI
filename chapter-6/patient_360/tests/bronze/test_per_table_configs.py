...
def test_ac1_filename_stems_match_lld_bronze_tasks() -> None:
    ...
    assert not missing and not extra, (
        f"Config stems do not match LLD §5.1. missing={sorted(missing)} extra={sorted(extra)}"
    )
...
def test_ac2_exactly_six_configs_use_fail() -> None:
    fail_stems = {p.stem for p in _config_files() if _load(p).get("empty_input_behavior") == "fail"}
...
def test_ac3_output_table_matches_catalog_schema_table(path: Path) -> None:
    ...
    assert schema == "bronze", (
        f"{path.name}: catalog_bronze_schema must default to 'bronze' per LLD §7.1, got {schema!r}"
    )
    ...
    assert output_table == f"{catalog}.{schema}.{table}", (
        f"{path.name}: output_table must equal '{catalog}.{schema}.{table}', got {output_table!r}"
    )
...
def test_referenced_contract_exists(path: Path) -> None:
    ...
    assert target.is_file(), (
        f"{path.name}: schema_ref points to {schema_ref!r} but that file does not exist"
    )
...
def test_metadata_columns_match_runner_contract(path: Path) -> None:
    cfg = _load(path)
    assert cfg.get("metadata_columns") == [
        "ds",
        "_ingested_at",
        "_source_batch_id",
        "_source_file",
    ], (
        f"{path.name}: metadata_columns must match the LLD §2.3 (change log "
        f"1.19) / runner contract "
        f"['ds', '_ingested_at', '_source_batch_id', '_source_file']"
    )
