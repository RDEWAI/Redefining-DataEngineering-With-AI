...
- **No surrogate key (determinism guard)**: No surrogate-key generation at all (no key to generate — the natural business key is the merge key). NEVER use `monotonically_increasing_id()` (non-deterministic) — kept as a determinism guard.
