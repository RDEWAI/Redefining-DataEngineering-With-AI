...
| 1.8 | 2026-05-20 | Scrum Master Agent | Status changed to Approved |
| 1.8 | 2026-05-20 | Scrum Master Agent | Re-baselined STORY-02-006 against LLD v1.14 (LLD-2026-05-20-patient-360.md) — added new AC capturing the upstream `bootstrap_se` SparkSubmitOperator (LLD §4.2) and removed the prior `max_active_tasks=1` mandate from AC4 since LLD v1.14 §4.1 restored DEV concurrency to 16 once `bootstrap_se` is wired upstream of the Bronze TaskGroup. Rationale: addresses LLD-DEVIATIONS row 6 (SE shared stats-table CREATE-race on cold-start + warm-restart `DELTA_CREATE_TABLE_WITH_NON_EMPTY_LOCATION`). New AC verifies (a) a SparkSubmitOperator task `bootstrap_se` exists, (b) its `application` points at `scripts/bootstrap_se_tables.py` (via `SE_BOOTSTRAP_APP` env var with the same default as LLD §4.2), and (c) `bootstrap_se >> bronze_tg` dependency wiring. AC4 updated: dropped `max_active_tasks=1`, replaced with `max_active_tasks` matching LLD §4.1 (DEV: 16) per v1.14 restoration. Verification block extended with matching grep checks; no other ACs, scope, dependencies, sprint, or status fields changed. Scenario B update: v1.7 → v1.8, new filename `BACKLOG-2026-05-20-patient-360.md`, prior file archived to `BACKLOG-2026-05-12-patient-360.md.bak`. Status reset to `Updated - Pending Review`. |
| 1.9 | 2026-05-21 | Scrum Master Agent | Reverted STORY-02-006 AC4 against LLD v1.15 (LLD-2026-05-21-patient-360.md) — mandates DEV `max_active_tasks=1` per LLD §4.1, citing the §13 Decision 12 Constraints paragraph added 2026-05-21 (embedded Derby Hive metastore is single-process; reproduced as `ERROR XSDB6: Another instance of Derby may have already booted the database null` when `max_active_tasks=16`). Updated AC4 Verification: required_grep for `max_active_tasks\s*[:=]\s*1\b` (matches both `=` and `:` forms); REMOVED the v1.8 `forbidden_grep` for `max_active_tasks=1` (now required, not forbidden) and REMOVED the v1.8 required_grep for `max_active_tasks.*16`. AC5 (`bootstrap_se` task) unchanged — bootstrap closes the SE-table CREATE race; the Derby cap closes the metastore-concurrency race; both gates are required together (rationale added to AC4 text). Description, Technical Notes, and Estimation Support re-cite LLD v1.15 §4.1 and §13 Decision 12 Constraints. Scope, dependencies, sprint, status, and other ACs unchanged. Scenario B update: v1.8 → v1.9, new filename `BACKLOG-2026-05-21-patient-360.md`, prior file archived to `BACKLOG-2026-05-20-patient-360.md.bak`. Status reset to `Updated - Pending Review`. |
| 1.9 | 2026-05-21 | Scrum Master Agent | Status changed to Approved |
| 1.10 | 2026-05-22 | Scrum Master Agent | STORY-02-006: added new AC explicitly listing `src/patient_360/bronze/reconciliation.py` as a deliverable of this story so future LLD pivots flow through to that file. AC text: file exists and honors LLD §13 Decision 12 — does NOT reference `uc_uri`, `UCSingleCatalog`, or `UC_URI` (UC is not in the runtime path per Decision 12 revised 2026-05-12); `build_spark()` is called without a `uc_uri` kwarg. Verification block extended: required_grep `def run_reconciliation_bronze` in `src/patient_360/bronze/reconciliation.py`; forbidden_grep `uc_uri` (+ `UCSingleCatalog`, `UC_URI`) in the same file. Renumbered prior validate-dag AC (AC6 → AC7) in both the AC list and the Verification block. Retrofit for LLD-DEVIATIONS row 8 + the 2026-05-22 direct-edit fix (reconciliation task previously failed at runtime with `TypeError: build_spark() got an unexpected keyword argument 'uc_uri'` after the §13 Decision 12 revision removed UC from the runtime path). Scope, dependencies, sprint, status, and other ACs unchanged. Scenario B update: v1.9 → v1.10, new filename `BACKLOG-2026-05-22-patient-360.md`, prior file archived to `BACKLOG-2026-05-21-patient-360.md.bak`. Status reset to `Updated - Pending Review`. |
| 1.11 | 2026-05-22 | Scrum Master Agent | STORY-03-001: added two new acceptance criteria (AC6, AC7) capturing the LLD-DEVIATIONS row 9 retrofit (Silver path anchoring + dq_rules_dir delegation). AC6 mandates `_bronze_path()` and `_silver_target_path()` (or equivalent path helpers) in `src/patient_360/silver/transform_patients.py` emit ABSOLUTE paths anchored on `PATIENT360_PROJECT_ROOT` (never bare-relative) — Airflow spark-submit CWD (`/opt/airflow`) is unpredictable and bare-relative paths 404; enforces LLD v1.15 §9.1. AC7 mandates `transform_patients.transform()` MUST NOT pass `dq_rules_dir` as a relative-path kwarg to `run_dq()` — either pass an absolute path resolved against `PATIENT360_PROJECT_ROOT`, or (preferred) OMIT the kwarg so `se_runner._resolve_dq_rules_dir()` falls back to the `DQ_RULES_DIR` env var (`/opt/patient_360/dq_rules` per docker-compose). Verification block extended: AC6 `required_grep` for `PATIENT360_PROJECT_ROOT` and `forbidden_grep` for the bare-relative form `"warehouse/{env.lower\(\)}/bronze"`; AC7 `forbidden_grep` for `dq_rules_dir=Path\(cfg\.get`. Retrofit for LLD-DEVIATIONS row 9 + the 2026-05-22 direct-edit fix (Silver SCD2 reads 404'd because path helpers returned relative paths; DQ rules skipped env-var fallback because the runner received an explicit relative `dq_rules_dir` kwarg). Scope, dependencies, sprint, status, and other ACs unchanged. Scenario C in-place edit (same-version-base same-date). |
| 1.11 | 2026-05-22 | Scrum Master Agent | Status changed to Approved |
| 1.12 | 2026-05-23 | Scrum Master Agent | Status changed to Approved |
| 1.13 | 2026-05-23 | Scrum Master Agent | **PIVOTED STORY-01-008 ACs for LLD v1.17 §13 Decision 17 Revised (2026-05-23) — Spark-DDL replaces Liquibase-over-UC-JDBC.** v1.16 plan was unbuildable because `io.unitycatalog:unitycatalog-jdbc` is not published on Maven Central. **REMOVED** v1.12 AC9 (`gen_liquibase_from_contracts.py` existence/greps — script will be deleted by next `update-scaffold`), AC11 (`liquibase-apply.sh` UC JDBC mode — Liquibase reverts to pre-v1.16 Postgres-only audit), AC14 (`unitycatalog-jdbc` driver pinning — artifact does not exist). **REWROTE** AC10 verification block (renumbered to AC9): `scripts/bootstrap_uc_tables.py` is now a one-shot **Spark application** (NOT a Liquibase wrapper) — boots a SparkSession with `UCSingleCatalog` (DDL-only binding, used only by this script — runtime tasks remain `DeltaCatalog` per LLD §13 Decision 12), reads `contracts/*.yml` directly, issues idempotent `CREATE SCHEMA IF NOT EXISTS` + `CREATE TABLE IF NOT EXISTS … USING DELTA LOCATION …` per contract. New required_grep: `SparkSession`, `UCSingleCatalog`, `CREATE SCHEMA IF NOT EXISTS`, `CREATE TABLE IF NOT EXISTS`, `USING DELTA`, `LOCATION`, `contracts/`. New forbidden_grep: `liquibase`, `JDBC`. **KEPT** AC12 (Makefile `bootstrap-uc`, renumbered to AC10) with description updated to call `python scripts/bootstrap_uc_tables.py` directly — no Liquibase invocation; verification extended with `forbidden_grep` for `bootstrap-uc:.*liquibase-apply|bootstrap-uc:.*gen_liquibase`. **KEPT** AC13 (canonical bring-up `make dev-up && make bootstrap-uc`, renumbered to AC11) unchanged. **ADDED** new AC12 — pipeline config keys `uc.bootstrap_catalog_name` (env var `UC_BOOTSTRAP_CATALOG_NAME`, default `unity`) and `uc.bootstrap_uri` (env var `UC_BOOTSTRAP_URI`, default `http://unity-catalog:8080`) per LLD v1.17 §7.1; verified in `_infra/cd/config/{DEV,STAGING,PROD}.yaml`. STORY-03-001 unchanged (runtime `CREATE TABLE` prohibition AC8-AC10 still hold verbatim — Spark-DDL pivot does not affect runtime writers). No story added/removed, no point changes (still 53 stories / 208 pts). Scenario C in-place edit (same date 2026-05-23, same v1). v1.12 archived to `BACKLOG-2026-05-23-patient-360.md.v1.12.bak`. Status reset to `Updated - Pending Review`. |
| 1.13 | 2026-05-23 | Scrum Master Agent | Status changed to Approved |
| 1.14 | 2026-05-23 | Scrum Master Agent | **PIVOTED STORY-01-008 ACs (SECOND PIVOT) for LLD v1.18 §13 Decision 17 Revised (2026-05-23 second pivot) — pure-Python REST client replaces Spark-DDL.** v1.17 Spark-DDL plan was unbuildable because `io.unitycatalog:unitycatalog-spark_2.13:0.3.0` is binary-incompat with Spark 4.x (`NoSuchMethodError: LogKey.$init$`). REST API verified working via curl against `/api/2.1/unity-catalog/{schemas,tables}`. **REWROTE AC9 verifications**: `scripts/bootstrap_uc_tables.py` is now a pure-Python httpx client (NO Spark, NO JDBC, NO UCSingleCatalog). New required_grep: `httpx` (or `urllib.request`), `/api/2.1/unity-catalog/schemas`, `/api/2.1/unity-catalog/tables`, `POST`, `GET`, `EXTERNAL`, `DELTA`, `storage_location`, `contracts/`. New forbidden_grep: `SparkSession`, `UCSingleCatalog`, `spark.sql`, `JDBC`, `liquibase`, `unitycatalog-spark`. For each populated contract: GET-before-POST against UC OSS schemas/tables endpoints; missing schemas POSTed with `{name, catalog_name}`; missing tables POSTed with `{name, catalog_name, schema_name, table_type=EXTERNAL, data_source_format=DELTA, columns[], storage_location}`. Idempotent. Logs each created/skipped table at INFO. Fail-loud at end if any registration failed. **UPDATED AC10**: Makefile `bootstrap-uc` now invokes plain `python scripts/bootstrap_uc_tables.py` (no spark-submit, no pyspark); extended `forbidden_grep` with `bootstrap-uc:.*spark-submit|bootstrap-uc:.*pyspark` to prevent regression. **KEPT AC11** unchanged. **UPDATED AC12**: added `uc.bootstrap_timeout_seconds` config key (env var `UC_BOOTSTRAP_TIMEOUT_SECONDS`, default `30`) per LLD v1.18 §7.1; flipped DEV `uc_bootstrap_uri` default expectation from `http://unity-catalog:8080` to `http://localhost:8080` because host-side `make bootstrap-uc` invocations cannot resolve the compose-internal hostname (`UnresolvedAddressException`). STAGING/PROD may keep `http://unity-catalog:8080` if running in-container — the LLD config files document the choice. Verification extended with required_grep for `localhost:8080` (DEV) and `uc_bootstrap_timeout_seconds` across all three env configs. STORY-03-001 unchanged (runtime `CREATE TABLE` prohibition AC8-AC10 still hold verbatim — REST pivot does not affect runtime writers). Executive Summary updated to cite LLD v1.18 + REST client. No story added/removed, no point changes (still 53 stories / 208 pts). Scenario C in-place edit (same date 2026-05-23, same v1). Status reset to `Updated - Pending Review`. |
| 1.14 | 2026-05-23 | Scrum Master Agent | Status changed to Approved |
| 1.15 | 2026-05-23 | Scrum Master Agent | Status changed to Approved |
| 1.15 | 2026-05-23 | Scrum Master Agent | ADDED STORY-03-001 AC11 — `src/patient_360/silver/transform_patients.py` MUST dedupe its source DataFrame to one row per natural key (`patient_id`) BEFORE invoking `apply_scd2`. Reproduced 2026-05-23 DAG retry (`manual_ld_20260523T222531Z`): `DELTA_MULTIPLE_SOURCE_ROW_MATCHING_TARGET_ROW_IN_MERGE` on second silver run. Root cause: bronze accumulates one snapshot per `ds` partition so the same patient appears N times across N partition days; transform reads all partitions without dedup; MERGE rejects multiple source rows hitting the same target. First-run cold-warehouse path masks the bug (`apply_scd2` uses `.write.format("delta").mode("overwrite").save(target_path)` — no MERGE, accepts duplicates); every subsequent run hits MERGE and fails. `apply_scd2` itself is correct — DMS §6 + LLD v1.18 §2.3 specify dedup-before-MERGE is the caller's responsibility. Recommended pattern: `row_number() OVER (PARTITION BY patient_id ORDER BY ds DESC) == 1`, fallback ordering by `_ingested_at`. Verification: `required_grep` for `dropDuplicates|row_number` + `patient_id` in `transform_patients.py`; new pytest `tests/silver/test_transform_patients_unit.py::test_source_dedup_keeps_one_per_natural_key` (3-row DataFrame, 2 rows sharing the same patient_id → asserts deduped output has exactly 1 row, latest by `ds`). STORY-01-008 unchanged; STORY-03-001 AC1-AC10 unchanged. No story added/removed, no point changes (still 53 stories / 208 pts). Scenario C in-place edit (same date 2026-05-23, same v1). Status reset to `Updated - Pending Review`. |
| 1.12 | 2026-05-23 | Scrum Master Agent | Re-baselined against LLD v1.16 (LLD-2026-05-23-patient-360.md) §13 Decision 17 — deploy-time UC table registration via Liquibase over the open-source UC JDBC driver. **STORY-01-008 (bootstrap-runtime)** extended with 6 new ACs (AC9-AC14) for the contract→Liquibase generator + UC bootstrap toolchain: `scripts/gen_liquibase_from_contracts.py` (overwrites `ddl/liquibase/changelogs/*.xml` from `contracts/*.yml`, idempotent + stable column order), `scripts/bootstrap_uc_tables.py` (UC health check + shells out to `liquibase-apply.sh`), `_infra/cd/liquibase-apply.sh` updated to UC JDBC defaults (`JDBC_URL=jdbc:unitycatalog://unity-catalog:8080`, `JDBC_DRIVER=io.unitycatalog.client.jdbc.UCDriver`) with Postgres fallback via `JDBC_FLAVOR=postgres`, `Makefile` target `bootstrap-uc` (gen + apply, idempotent via DATABASECHANGELOG), canonical bring-up `make dev-up && make bootstrap-uc` documented, Liquibase container mounts `io.unitycatalog:unitycatalog-jdbc` driver JAR. **STORY-03-001 (silver-dim-patients)** extended with 3 new ACs (AC8-AC10): `transform_patients.py` MUST NOT call `CREATE TABLE`/`CREATE SCHEMA`/catalog DDL (path-based `.save()` only via DeltaCatalog), `utils/scd2.py::apply_scd2` MUST NOT call `CREATE TABLE` (`target_table` kwarg removed per LLD v1.16 §2.3; first-run uses `.format("delta").mode("overwrite").save(target_path)`; MERGE uses `delta.\`<target_path>\`` syntax), lock-in unit test in `tests/silver/test_transform_patients.py` greps both files and FAILS if `CREATE TABLE`/`CREATE SCHEMA`/`saveAsTable(` appear. Both stories cross-reference LLD v1.16 §13 Decision 17 and `chapter-6/developer-plugin/LLD-DEVIATIONS.md` row 10 (forward reference). Folded the contract-generator work into STORY-01-008 per user direction (keeps EPIC-01 size manageable; no new story created). No story added/removed, no point changes (still 53 stories / 208 pts). Scenario B update: v1.11 → v1.12, new filename `BACKLOG-2026-05-23-patient-360.md`, prior file archived to `BACKLOG-2026-05-22-patient-360.md.bak`. Status reset to `Updated - Pending Review`. |
