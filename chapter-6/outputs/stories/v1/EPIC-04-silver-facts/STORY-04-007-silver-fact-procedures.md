...
Implement `src/patient_360/silver/transform_procedures.py` per LLD §5.2. Reads `unity.bronze.synthea_procedures` (UC-managed), enriches via FK joins to Silver dims (`is_current=TRUE`), applies STM Tab:Bronze-to-Silver transforms, then `insertInto`s the Liquibase-pre-created `unity.silver.clinical_procedures` UC table — `df.write.mode("overwrite").insertInto("unity.silver.clinical_procedures")` under dynamic partition overwrite (`spark.sql.sources.partitionOverwriteMode=dynamic`; idempotent per-`ds`, NOT `replaceWhere` which `insertInto` silently ignores — re-runs would append/double) (Decision 12/15 re-adopted 2026-06-18; never `saveAsTable`/path-based `.save`) (LLD §3.3). Inline SE per `dq_rules/clinical_procedures.yml`. Empty-input behavior: `fail` for `encounters` and `allergies` (LLD §5.2), `write_empty` otherwise.
