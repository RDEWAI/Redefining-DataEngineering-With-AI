...
- [ ] Output written full-overwrite via `insertInto("unity.gold.patient_billing_summary")` (pre-created UC table); no `saveAsTable`/path-based `.save` (Decision 12/15) [LLD §3.3]
...
AC2:
  - grep: {glob: "patient_360/src/patient_360/gold/build_*.py", pattern: "insertInto.*unity\\.gold\\.patient_billing_summary|unity\\.gold\\.patient_billing_summary"}
