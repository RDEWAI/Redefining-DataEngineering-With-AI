...
- [ ] All 13 configs declare `output_table: unity.bronze.synthea_{table}` (the pre-created UC EXTERNAL Delta target the runner `insertInto`s); **no** path-based `output_path: .../warehouse/.../bronze/...` (UC `insertInto` re-adopted per LLD §13 Decision 12 + 15, 2026-06-18) [LLD §13 Decision 12/15, §9.1]
...
- **Upstream references**: LLD §5.1, §13 Decision 12/15 (re-adopted 2026-06-18)
...
AC3:
  - grep_count: {glob: "patient_360/airflow/configs/*.yml", pattern: 'output_table:\s*unity\.bronze\.synthea_', min: 13}
  - forbidden_grep: {glob: "patient_360/airflow/configs/*.yml", pattern: 'output_path:.*warehouse/.*/bronze/', reason: "Bronze writes are insertInto unity.bronze.<table>, not path-based output_path per LLD §13 Decision 12/15 (re-adopted 2026-06-18)"}
AC4:
