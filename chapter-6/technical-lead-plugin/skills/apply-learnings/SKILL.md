...
```bash
cat memory/lld/learnings-queue.jsonl
```
...
```
Pending learnings:
- create-lld: 3 corrections
- validate-lld: 1 correction
```
...
```
Proposed learning for create-lld:
- **L-003** (2026-03-22): Always specify empty-input behavior for every DAG task — never assume "skip" is the default. Example: "When input path is empty, write zero-row Parquet with schema."
...
1. Read the target skill file:
   `technical-lead-plugin/skills/{skill-name}/SKILL.md`
...
After all learnings are processed, update `memory/lld/learnings-queue.jsonl`:
