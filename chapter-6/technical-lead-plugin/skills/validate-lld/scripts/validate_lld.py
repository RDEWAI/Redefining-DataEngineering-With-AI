...
def check_deployment_infra_paths(sections: dict[str, str]) -> list[ValidationResult]:
    """WARNING: §9 must reference _infra/ci/, _infra/cd/, _infra/docker/, ddl/migrations/."""
    results: list[ValidationResult] = []
    content = sections.get("9. Deployment", "")
    if not content:
        return results
    required_refs = ["_infra/ci", "_infra/cd", "_infra/docker", "ddl/migrations"]
        ...
        results.append(
            ValidationResult(
                ...
                suggestion=(
                    ...
                    "9.3 `_infra/docker/`, 9.4 `ddl/migrations/`."
                ),
            )
        )
    return results
