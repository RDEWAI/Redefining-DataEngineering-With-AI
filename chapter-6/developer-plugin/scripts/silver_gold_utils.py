...
def extract_dms_columns(dms_path: Path, table: str) -> list[str]:
    ...
    Locates the heading mentioning ``table`` and parses the column names from
    the section that follows. Two schema encodings are supported:

    * **Fenced YAML blocks** (DMS §3 Silver and §4 Gold) — ```yaml blocks with
      a ``columns:`` list whose entries each carry a ``name:`` field. This is
      the canonical encoding for Silver/Gold schemas.
    * **Markdown pipe tables** — legacy/alternate encoding where the first
      pipe cell of each non-header row is the column name.

    Returns column names only; types are not extracted here (the contract YAML
    carries them).
    """
    ...
    # --- Fenced YAML blocks (```yaml ... ```) ---
    for block in re.findall(r"```ya?ml\s*\n(.*?)```", section, re.DOTALL):
        parsed = yaml.safe_load(block)
        if not isinstance(parsed, dict):
            continue
        # Only trust a block that actually describes this table.
        if str(parsed.get("table", table)) != table:
            continue
...
def extract_dms_hash_columns(dms_path: Path, table: str) -> list[str]:
    """Return the SCD2 Type-2 tracked column list for a dim from DMS §6.

    Parses the DMS §6 ``Dimension Attribute SCD Types`` markdown table. That
    table has one row per (dimension, attribute-group) pair with columns
    ``| Dimension | Attribute | SCD Type | ... |``. A single dimension may span
    several rows. For the requested ``table`` this unions the ``Attribute``
    cells of every row whose ``SCD Type`` cell is **Type 2**, and explicitly
    excludes Type 1 rows. The resulting attribute names are the columns hashed
    into ``_record_hash`` for change detection.

    Returns column names in document order with duplicates removed.
    """
    text = dms_path.read_text(encoding="utf-8")
    ...
    for line in section.splitlines():
        ...
        if len(cells) < 3:
            continue
        dimension, attribute, scd_type = cells[0], cells[1], cells[2]
        # Skip the header row.
        if dimension.lower() in {"dimension", ""}:
            continue
        if dimension != table:
            continue
        # Only Type 2 rows; exclude Type 1 (and anything not Type 2).
        if not re.search(r"\btype\s*2\b", scd_type, re.IGNORECASE):
            continue
        for name in re.split(r",\s*", attribute):
