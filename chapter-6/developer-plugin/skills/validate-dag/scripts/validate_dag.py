...
- ``UC-WIRING-001`` — any Bronze runner under ``src/**/bronze/**.py``
  using ``saveAsTable(...)`` (the UCSingleCatalog-unsupported staged
  create) or a path-based ``df.write.format("delta").save(...)`` for the
  data table is rejected. Bronze writes MUST go via
  ``insertInto("unity.bronze.<table>")`` into the Liquibase-pre-created
  EXTERNAL Delta table (LLD v1.13 UC write pattern).
...
# UC-WIRING-001: `saveAsTable(...)` — the UCSingleCatalog-unsupported staged
# create. Forbidden in Bronze runners; writes must use insertInto instead.
SAVE_AS_TABLE_WRITE = re.compile(r"\.saveAsTable\s*\(")
# UC-WIRING-001 (companion): path-based `format("delta").save(...)` for the
# data table — also forbidden; the table is pre-created EXTERNAL by Liquibase.
...
def check_file(path: Path) -> list[Finding]:
    ...
    for lineno, line in enumerate(text.splitlines(), start=1):
        ...
        if is_bronze and SAVE_AS_TABLE_WRITE.search(line):
            findings.append(
                Finding(
                    level=Level.CRITICAL,
                    rule="UC-WIRING-001",
                    ...
                    line=lineno,
                    message=(
                        "saveAsTable(...) in a Bronze runner — UCSingleCatalog "
                        "does not support the staged-create that saveAsTable "
                        "performs. Bronze writes must target a Liquibase-pre-"
                        "created EXTERNAL Delta table (LLD v1.13 UC write pattern)."
                    ),
                    suggestion=(
                        "use insertInto into the Liquibase-pre-created "
                        "unity.bronze.<table>; never saveAsTable"
                    ),
                )
            )
        if is_bronze and PATH_BASED_DELTA_WRITE.search(line):
            findings.append(
                Finding(
                    ...
                    line=lineno,
                    message=(
                        "Path-based Delta write (.save(path)) of the data table "
                        "in a Bronze runner — the table is pre-created EXTERNAL "
                        "by Liquibase; runtime writes must go through Unity "
                        "Catalog (LLD v1.13 UC write pattern)."
                    ),
                    suggestion=(
                        "use insertInto into the Liquibase-pre-created "
                        "unity.bronze.<table>; never saveAsTable"
