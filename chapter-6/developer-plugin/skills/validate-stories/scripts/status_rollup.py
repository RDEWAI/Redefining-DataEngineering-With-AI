...
CLASSIFIER_RULES: list[tuple[str, re.Pattern[str]]] = [
    ...
    (
        "silver",
        re.compile(
            # Silver transforms: SCD2 dimension / cleansed source files under a
            # silver package. create-silver owns these (not create-ingestion).
            r"/silver/[^/\s`]+\.py|transform_[^/\s`]+_silver\b",
            re.IGNORECASE,
        ),
    ),
    (
        "gold",
        re.compile(
            # Gold marts / consumer aggregates under a gold package.
            r"/gold/[^/\s`]+\.py|build_[^/\s`]+_gold\b",
            re.IGNORECASE,
        ),
    ),
    (
