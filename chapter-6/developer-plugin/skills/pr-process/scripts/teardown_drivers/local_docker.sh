...
case "$MODE" in
  ...
  --destroy)
    ...
    if ! docker compose -f "$COMPOSE_FILE" down -v --remove-orphans >/dev/null 2>&1; then
      # Surface the error but try the prune anyway — partial cleanup is
      # still better than nothing.
      err=$(docker compose -f "$COMPOSE_FILE" down -v --remove-orphans 2>&1 || true)
      cat >&2 <<EOF
