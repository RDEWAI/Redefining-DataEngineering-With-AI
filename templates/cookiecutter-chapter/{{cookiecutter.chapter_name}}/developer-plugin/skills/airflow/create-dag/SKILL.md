...
- DAG config from `{{cookiecutter.project_name}}/airflow/configs/` if present
- Existing DAGs in `{{cookiecutter.project_name}}/airflow/dags/` for patterns
...
Save to `{{cookiecutter.project_name}}/airflow/dags/{dag_id}.py`
