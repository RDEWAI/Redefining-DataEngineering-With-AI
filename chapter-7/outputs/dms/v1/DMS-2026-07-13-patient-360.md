...
erDiagram
  ...
  clinical_encounters ||--o{ billing_claims : "appointment_id"
...
columns:
  ...
  - name: REACTION1
    ...
    description: Primary reaction description text (e.g. 'Reaction to Penicillin') — source emits text, not a SNOMED code
  - name: DESCRIPTION1
    type: VARCHAR
    nullable: true
  ...
  - name: REACTION2
    ...
    description: Secondary reaction description text — source emits text, not a SNOMED code
  - name: DESCRIPTION2
    type: VARCHAR
    nullable: true
...
columns:
  ...
  - name: DEPARTMENTID
    ...
    description: Department ID — source emits a UUID (e.g. '2d236e35-5536-48d6-af82-10be3ef166f7'), not a numeric ID
  - name: PATIENTDEPARTMENTID
    type: VARCHAR
    nullable: true
    source: synthea.claims.PATIENTDEPARTMENTID
    description: Patient department ID — source emits a UUID, not a numeric ID
  - name: DIAGNOSIS1
    type: BIGINT
    nullable: true
...
columns:
  ...
  - name: appointment_id
    ...
    description: FK to clinical_encounters.encounter_id — the claim-to-encounter linkage (one claim per encounter/appointment). Required by the Gold patient_billing_summary encounter-grain join (DMS §4 / STM Silver-to-Gold).
  - name: provider_id
    type: VARCHAR
    nullable: true
  ...
  - name: primary_payer_id
    ...
    description: FK to reference_payers.payer_id — primary payer for the claim
  - name: secondary_payer_id
    type: VARCHAR
    nullable: true
    source: bronze.synthea_claims.SECONDARYPATIENTINSURANCEID
    description: FK to reference_payers.payer_id — secondary payer for the claim
...
| 1.1 | 2026-06-15 | Data Modeler Agent | Status changed to Approved |
| 1.2 | 2026-06-18 | Data Modeler Agent | Source-data-type corrections (Synthea emits text, not numeric) discovered when Bronze moved to schema-enforced insertInto into pre-created UC tables, which failed with CAST_INVALID_INPUT. Retyped 4 Bronze columns BIGINT → VARCHAR: synthea_allergies.REACTION1 and REACTION2 (reaction description text, not SNOMED codes), synthea_claims.DEPARTMENTID and PATIENTDEPARTMENTID (UUIDs, not numeric IDs). Silver columns unchanged (already VARCHAR). Re-approval to follow. |
| 1.2 | 2026-06-18 | Data Modeler Agent | Status changed to Approved |
| 1.3 | 2026-07-13 | Data Modeler Agent | Silver §3.10 billing_claims: (1) Added `appointment_id` column (VARCHAR, source bronze.synthea_claims.APPOINTMENTID → verified in raw claims.csv col 17) as FK to clinical_encounters.encounter_id, required by the Gold patient_billing_summary encounter-grain join. (2) Renamed `primary_insurance_id` → `primary_payer_id` and `secondary_insurance_id` → `secondary_payer_id` to reconcile Silver payer FK naming with the §4 Gold patient_billing_summary and the reference_payers natural key `payer_id`; sources unchanged (PRIMARY/SECONDARYPATIENTINSURANCEID), descriptions updated to FK-to-reference_payers.payer_id. Updated billing_claims foreign_keys block and the §1.4 ER diagram (added appointment_id + clinical_encounters relationship). Re-approval to follow. |
| 1.3 | 2026-07-13 | Data Modeler Agent | Status changed to Approved |
