...
As a data engineer, I want transform the bronze `unity.bronze.synthea_patients` UC table into Silver dimension `unity.silver.clinical_patients` with SCD Type 2 so that the patients dimension preserves history and Gold tables get the latest version via `is_current=TRUE`.

## Description

Implement `src/patient_360/silver/transform_patients.py` per LLD §5.2 / DMS §6. Reads the bronze table via `spark.read.table("unity.bronze.synthea_patients")` (Decision 12 & 15 re-adopted 2026-06-18 — UC is the runtime catalog), drops PHI columns per DMS §3 (LLD §5.2), applies derived fields, then calls `apply_scd2(...)` from `utils/scd2.py` which `MERGE INTO`s the `ddl/migrations/*.sql`-pre-created `unity.silver.clinical_patients` UC table (`DeltaTable.forName`, not a filesystem path). PHI fields like `SSN`, `DRIVERS`, `PASSPORT` (where applicable) MUST be dropped at this Silver boundary (LLD §5.2, NFR-6). Inline SE via `se_runner.run_dq(...)`.

## Acceptance Criteria


- [ ] `transform_patients.py` reads via `spark.read.table("unity.bronze.synthea_patients")` and SCD2-`MERGE INTO`s the pre-created `unity.silver.clinical_patients` UC table (Decision 12 & 15 re-adopted 2026-06-18 — UC is the runtime catalog) [LLD §5.2]
...
- **Implementation hints**: Reuse `apply_scd2` from `utils/scd2.py` (STORY-01-003); it `MERGE INTO`s the named `unity.silver.clinical_patients` table via `DeltaTable.forName(spark, "unity.silver.clinical_patients")`. The table is pre-created by the `ddl/migrations/*.sql` migrations (`make ddl-apply`, beeline against the Spark Thrift Server); SCD2 dims have no `ds` partition per LLD §3.3.
