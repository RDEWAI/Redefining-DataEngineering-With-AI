...
Implement `src/patient_360/gold/build_summary.py` (or `build_patient_summary.py`) per LLD §5.3. Reads current-version `unity.silver.clinical_patients` (`is_current=TRUE`) via `spark.read.table(...)`, joins to relevant Silver facts, denormalizes via ARRAY<STRUCT> (LLD §5.3 / NFR-1), then full-overwrite `insertInto`s the `ddl/migrations/*.sql`-pre-created `unity.gold.patient_summary` UC table — `df.write.mode('overwrite').insertInto("unity.gold.patient_summary")` (Decision 12/15 re-adopted 2026-06-18; never `saveAsTable`/path-based `.save`). Inline SE via `dq_rules/patient_summary.yml`. Empty-input: `fail` (LLD §5.3 — consumer table must have data).
...
- [ ] Output written full-overwrite via `insertInto("unity.gold.patient_summary")` (pre-created UC table); no `saveAsTable`/path-based `.save` (Decision 12/15) [LLD §3.3]
...
AC2:
  - grep: {glob: "patient_360/src/patient_360/gold/build_*.py", pattern: "insertInto.*unity\\.gold\\.patient_summary|unity\\.gold\\.patient_summary"}
