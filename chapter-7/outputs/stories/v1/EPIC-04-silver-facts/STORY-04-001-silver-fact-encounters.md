...
Implement `src/patient_360/silver/transform_encounters.py` per LLD §5.2. Reads `unity.bronze.synthea_encounters` (UC-managed), enriches via FK joins to Silver dims (`is_current=TRUE`), applies STM Tab:Bronze-to-Silver transforms, then `insertInto`s the `ddl/migrations/*.sql`-pre-created `unity.silver.clinical_encounters` UC table — `df.write.mode("overwrite").insertInto("unity.silver.clinical_encounters")` under dynamic partition overwrite (`spark.sql.sources.partitionOverwriteMode=dynamic`; idempotent per-`ds`, NOT `replaceWhere` which `insertInto` silently ignores — re-runs would append/double) (Decision 12/15 re-adopted 2026-06-18; never `saveAsTable`/path-based `.save`) (LLD §3.3). Inline SE per `dq_rules/clinical_encounters.yml`. Empty-input behavior: `fail` for `encounters` and `allergies` (LLD §5.2), `write_empty` otherwise.
...
- [ ] Output written via `df.write.mode("overwrite").insertInto("unity.silver.clinical_encounters")` (pre-created UC table); idempotency via dynamic partition overwrite (`spark.sql.sources.partitionOverwriteMode=dynamic`), NOT `replaceWhere` (`insertInto` silently ignores it — re-runs would append/double); no `saveAsTable`/path-based `.save` (LLD §4.5 idempotency, Decision 12/15) [LLD §3.3, §4.5, §13 Decision 15]
...
AC3:
  - grep: {file: "patient_360/src/patient_360/silver/transform_encounters.py", pattern: "mode\\(['\"]overwrite['\"]\\)\\.insertInto|\\.insertInto"}
  - forbidden_grep: {file: "patient_360/src/patient_360/silver/transform_encounters.py", pattern: "replaceWhere", reason: "insertInto silently ignores replaceWhere — re-runs append/double. Idempotency is dynamic partition overwrite per LLD §13 Decision 15 (re-adopted 2026-06-18)"}
  - grep: {file: "patient_360/src/patient_360/silver/transform_encounters.py", pattern: "insertInto.*unity\\.silver\\.clinical_encounters|unity\\.silver\\.clinical_encounters"}
  - forbidden_grep: {file: "patient_360/src/patient_360/silver/transform_encounters.py", pattern: "saveAsTable|\\.save\\(\\s*f?['\"].*warehouse", reason: "Silver facts insertInto pre-created unity.silver.<table>; no saveAsTable/path-based .save per LLD §13 Decision 12/15 (re-adopted 2026-06-18)"}
AC4:
