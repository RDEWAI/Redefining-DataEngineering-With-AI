...
Implement `src/patient_360/silver/transform_immunizations.py` per LLD §5.2. Reads `unity.bronze.synthea_immunizations` (UC-managed), enriches via FK joins to Silver dims (`is_current=TRUE`), applies STM Tab:Bronze-to-Silver transforms, then `insertInto`s the `ddl/migrations/*.sql`-pre-created `unity.silver.clinical_immunizations` UC table — `df.write.mode("overwrite").insertInto("unity.silver.clinical_immunizations")` under dynamic partition overwrite (`spark.sql.sources.partitionOverwriteMode=dynamic`; idempotent per-`ds`, NOT `replaceWhere` which `insertInto` silently ignores — re-runs would append/double) (Decision 12/15 re-adopted 2026-06-18; never `saveAsTable`/path-based `.save`) (LLD §3.3). Inline SE per `dq_rules/clinical_immunizations.yml`. Empty-input behavior: `fail` for `encounters` and `allergies` (LLD §5.2), `write_empty` otherwise.
