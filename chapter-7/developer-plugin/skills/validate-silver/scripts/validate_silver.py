...
``developer-plugin/skills/validate-silver/SKILL.md``.
...
# DMS §3 SCD2 metadata columns — the EXACT four (no surrogate_key, no expiry_date,
# no record_hash without underscore, no dw_* columns). Natural key + effective_from
# form the PK; the natural key itself is an ordinary business column already covered
# by R6's DMS §3 set-equality check, so it is intentionally NOT repeated here.
SCD2_MANDATED_COLS = {
    "effective_from",
    "effective_to",
    "is_current",
...
def check_schema_alignment(
    ...
    for task in tasks:
        ...
        # R7: SCD2 metadata columns — a subset of the DMS §3 column set already
        # checked by R6. Kept as a redundant, SCD2-scoped guard so a contract that
        # silently drops effective_from/effective_to/is_current/_record_hash is
        # flagged explicitly rather than buried in R6's generic missing-columns list.
        if silver_table in SCD2_TABLES:
            ...
            if scd2_missing:
                findings.add(
                    "WARNING",
                    "R7",
                    f"{silver_table} (SCD2): missing DMS §3 metadata columns {sorted(scd2_missing)}",
                )
        # R8: tags present
...
def check_scd2_wiring(
    ...
    for task in tasks:
        ...
        if not scd2_calls:
            ...
            continue
        # R10: the SCD2 dim target must be a NAMED Unity Catalog table
        # (DeltaTable.forName("unity.silver.<dim>") / a table FQN arg) — NOT a
        # filesystem path. Tables are pre-created EXTERNAL Delta by Liquibase;
        # the helper MERGEs into the named UC table (LLD v1.13 UC write pattern).
        # kwargs cover target_table + natural_keys + hash_columns + effective_date.
        for call in scd2_calls:
            kw = call_kwargs(call)
            required = {"target_table", "natural_keys", "hash_columns", "effective_date"}
            ...
            # R10: the target must name a UC table (unity.silver.<dim>), not a
            # filesystem path. Flag a target argument that looks like a path.
            target_node = kw.get("target_table")
            if isinstance(target_node, ast.Constant) and isinstance(target_node.value, str):
                tgt = target_node.value
                if "/" in tgt or "\\" in tgt or tgt.startswith("delta."):
                    findings.add(
                        "CRITICAL",
                        "R10",
                        f"{table}: apply_scd2 target_table='{tgt}' is a filesystem "
                        f"path — must be a named UC table (unity.silver.<dim>)",
                    )
                elif not tgt.startswith("unity.silver."):
                    findings.add(
                        "WARNING",
                        "R10",
                        f"{table}: apply_scd2 target_table='{tgt}' is not a "
                        f"unity.silver.<dim> FQN",
                    )

            # R11: hash_columns list literal matches the DMS §6 Type-2 tracked
            # columns for this table. extract_dms_hash_columns parses the DMS §6
            # "Dimension Attribute SCD Types" table and returns the Type-2 columns;
            # when it returns a non-empty list we assert set-equality against the
            # module's hash_columns literal. (The parser itself is owned by
            # silver_gold_utils.py — not modified here.)
            # NOTE: runs per apply_scd2 call inside the loop — NOT gated on the
            # forbidden saveAsTable branch (that bug made R11 dead code on correct
            # UC-aligned modules).
        ...
        # R10: SCD2 dims must never use saveAsTable (UCSingleCatalog-unsupported
        # staged create). The merge targets a Liquibase-pre-created UC table.
        if find_calls(tree, "saveAsTable"):
            findings.add(
                "CRITICAL",
                "R10",
                f"{table}: SCD2 module uses saveAsTable — forbidden; the dim is a "
                f"Liquibase-pre-created UC table merged via DeltaTable.forName",
            )
...
def check_dq_gate(
    ...
    for task in tasks:
        ...
        # R18: local dq_rules/<table>.yml rules: subtree matches the upstream DQS
        # se-rules file. Upstream files use the se-rules-<table-with-hyphens>.yaml
        # convention (GROUND TRUTH #4) — NOT <table>.yaml. We compare only the
        # `rules:` subtree semantically (env/table_name headers legitimately differ
        # between the upstream template and the local copy), not byte-for-byte.
        if upstream_dqs_dir is not None:
            local_yml = project_root / "dq_rules" / f"{silver_table}.yml"
            hyphenated = silver_table.replace("_", "-")
            upstream_yml = upstream_dqs_dir / f"se-rules-{hyphenated}.yaml"
            if upstream_yml.exists() and _dq_rules_subtree_diff(local_yml, upstream_yml):
                findings.add(
                    "WARNING",
                    "R18",
                    f"{silver_table}: dq_rules 'rules:' subtree diverged from upstream "
                    f"DQS se-rules ({upstream_yml.name})",
                )

        # R20: dq_env.<ENV>.table_name must be the fully-qualified
        # `unity.silver.<table>` (3-part) — UC 0.5.0 managed-audit-table FQN
        # convention. se_runner reads it as the SE rule-filter key AND the base
        # for the managed <name>_error/_stats FQNs; a bare/2-part name
        # reproduces the UC empty-namespace `fullTableNameForApi` AIOOBE.
        local_yml = project_root / "dq_rules" / f"{silver_table}.yml"
        if local_yml.exists():
            doc = load_yaml(local_yml) or {}
            for env_name, blk in (doc.get("dq_env") or {}).items():
                if not isinstance(blk, dict):
                    continue
                tn = blk.get("table_name")
                if tn is not None and tn.count(".") < 2:
                    findings.add(
                        "CRITICAL",
                        "R20",
                        f"{silver_table}: dq_env.{env_name}.table_name='{tn}' must be "
                        f"a fully-qualified `unity.silver.<table>` (3-part), not bare",
                    )
...
def _dq_rules_subtree_diff(local_yml: Path, upstream_yml: Path) -> bool:
    """True if the ``rules:`` subtree of the two YAML files differs semantically.

    Only the ``rules:`` list is compared — the ``dq_env`` / ``table_name`` headers
    legitimately differ between the upstream DQS template and the local copy, so a
    byte-for-byte comparison would false-positive. Returns True (divergence) if
    either file is missing or unparseable.
    """
    local = load_yaml(local_yml)
    upstream = load_yaml(upstream_yml)
    if not isinstance(local, dict) or not isinstance(upstream, dict):
        return True
    return local.get("rules") != upstream.get("rules")
...
def check_dag_wiring(project_root: Path, tasks: list[dict], findings: Findings) -> None:
    ...
    # Expected Spark-touching Silver task IDs (all transform_<table>_silver tasks).
    # Reconciliation tasks (reconciliation_silver, etc.) are SQL-only and may
    # legitimately be PythonOperator / SQLExecuteQueryOperator per LLD §4.2.
    ...
    spark_touching_task_ids = expected_silver_dim_tasks | expected_silver_fact_tasks

    # Find operator constructions; flag a PythonOperator only when its task_id is a
    # Spark-touching Silver transform task (not a blanket count — see R23 below).
    operator_count = 0
    python_op_on_spark_task: set[str] = set()
    for node in ast.walk(tree):
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
            if node.func.id == "SparkSubmitOperator":
                ...
                if isinstance(tid, str) and tid in spark_touching_task_ids:
                    python_op_on_spark_task.add(tid)
    ...
    # R23: PythonOperator is banned ONLY for Spark-touching Silver transform tasks
    # (IL-011). SQL-only reconciliation tasks may use PythonOperator legitimately.
    if python_op_on_spark_task:
        findings.add(
            "CRITICAL",
            "R23",
            f"Spark-touching Silver tasks declared as PythonOperator (use SparkSubmitOperator, IL-011): "
            f"{sorted(python_op_on_spark_task)}",
        )
