...
description: >
  ...
  - Wire up _infra/cd/ scripts (DAG sync, DDL apply) for local
...
- `ddl/migrations/**` (per-table plain `.sql` owned by layer skills; no
  master/Liquibase — applied by `ddl-apply.sh`, L-015)
...
1. Read the resolved STORY-NN-NNN markdown and extract:
   ...
   - **The deploy commands** the AC names (e.g. `make ddl-apply` /
     `ddl-apply.sh` for the plain-`.sql` DDL pre-create, `airflow dags
     reserialize`, `airflow dags list-import-errors`, `docker compose exec …`)
     — these are the verbs the smoke test will assert against.
...
| `ddl-apply.sh` (or similarly named DDL apply) | Apply the project's plain dated `.sql` migrations under `ddl/migrations/` against the Spark Thrift Server via beeline — i.e. invoke `make ddl-apply` (which runs `_infra/docker/ddl-apply.sh`). Liquibase was REMOVED (developer-plugin L-015: it cannot work on UC OSS + Spark Thrift); there is NO Liquibase container, `master-changelog.xml`, or `ddl/liquibase/`. |
...
- One test per AC, named to mirror the AC's intent. Suggested map:
  ...
  |---|---|---|
  | `ddl-apply ... pre-creates tables locally` | `test_ddl_apply` | Run `make ddl-apply` (or the `_infra/docker/ddl-apply.sh` one-shot) via `subprocess.run`; assert `returncode == 0` and the expected `unity.<schema>.<table>` tables resolve. |
...
3. The DDL-apply step MUST run the plain `.sql` migrations under
   `ddl/migrations/` via `make ddl-apply` (beeline). There is no Liquibase,
   `master-changelog.xml`, or aggregator — the dated/sequenced filenames are
   the ordering (L-015).
