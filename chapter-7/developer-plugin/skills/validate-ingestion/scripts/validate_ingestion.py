...
def check_yaml_config(
    ...
    # The Bronze table is pre-created EXTERNAL Delta by Liquibase; the per-table
    # config must declare the UC table FQN (output_table: unity.bronze.<table>),
    # NOT a filesystem output_path. Stay lenient when neither key is clear.
    output_table = data.get("output_table")
    if isinstance(output_table, str) and output_table.strip():
        ot = output_table.strip()
        if not ot.startswith("unity.bronze."):
            findings.critical.append(
                f"{yaml_path}: output_table `{ot}` must name the Liquibase-pre-"
                f"created UC table as `unity.bronze.<table>` (LLD v1.13 UC write "
                f"pattern), not a filesystem path"
            )
        elif declared_table and not (
            ot == f"unity.bronze.{declared_table}"
            or ot.endswith(f".{declared_table}")
        ):
            findings.warning.append(
                f"{yaml_path}: output_table `{ot}` does not match "
                f"`unity.bronze.{declared_table}`"
            )
    elif "output_path" in data:
        # Legacy path-based config — flag the migration to the UC table FQN.
        findings.critical.append(
            f"{yaml_path}: uses path-based `output_path` — replace with "
            f"`output_table: unity.bronze.<table>` (the table is pre-created "
            f"EXTERNAL Delta by Liquibase; writes go via insertInto)"
        )
...
def check_dq_rules_schema(dq_path: Path, findings: Findings) -> None:
    ...
    if is_se_format:
        ...
        if not isinstance(dq_env, dict) or not dq_env:
            ...
            # FQN convention (UC 0.5.0 managed audit tables): every
            # dq_env.<ENV>.table_name MUST be a fully-qualified
            # `unity.<schema>.<table>` (3-part). se_runner reads it as the SE
            # rule-filter key AND derives the managed `<name>_error`/`_stats`
            # table FQNs from it; a bare name reproduces the UC empty-namespace
            # `fullTableNameForApi` AIOOBE on the error-table write.
            for env_name, blk in dq_env.items():
                if not isinstance(blk, dict):
                    continue
                tn = blk.get("table_name")
                if tn is not None and tn.count(".") < 2:
                    findings.critical.append(
                        f"{dq_path}: dq_env.{env_name}.table_name='{tn}' must be a "
                        f"fully-qualified `unity.<schema>.<table>` (3-part), not a "
                        f"bare/2-part name"
                    )
...
def resolve_lld_path(workspace_root: Path) -> Path | None:
    ...
    fatal.
    """
    # All upstream artifacts (LLD/DMS/STM/DQS/HLD/DRD, plus stories) live under
