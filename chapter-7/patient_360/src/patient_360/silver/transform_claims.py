...
STM: Tab:Bronze-to-Silver (claims) rows 151-167 (STM v3.1 — appointment_id
     row 156, primary_payer_id row 154, secondary_payer_id row 155).
DMS: §3.10 billing_claims schema (AUTHORITATIVE Silver column contract). Updated
     for DMS v1.3 (2026-07-13): added appointment_id (bronze APPOINTMENTID, FK
     clinical_encounters.encounter_id) and renamed the payer FK columns to
     primary_payer_id / secondary_payer_id (bronze sources
     PRIMARY/SECONDARYPATIENTINSURANCEID unchanged; FK to
     reference_payers.payer_id). The extra DMS columns (diagnosis_code_3,
     referring/supervising_provider_id) map directly from the bronze source
     columns DIAGNOSIS3 / REFERRINGPROVIDERID / SUPERVISINGPROVIDERID present in
     synthea_claims.
...
def _cleanse(bronze_df: DataFrame, ds: str) -> DataFrame:
    df = bronze_df.select(
        ...
        F.trim(F.col("APPOINTMENTID")).alias("appointment_id"),  # row 156
        F.trim(F.col("PROVIDERID")).alias("provider_id"),  # row 153
        F.trim(F.col("PRIMARYPATIENTINSURANCEID")).alias("primary_payer_id"),  # row 154
        F.trim(F.col("SECONDARYPATIENTINSURANCEID")).alias("secondary_payer_id"),  # row 155
