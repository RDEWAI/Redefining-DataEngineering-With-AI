...
## Inline data quality (Spark Expectations)

Every Bronze (and Silver) write is gated inline by Nike Spark Expectations via
`src/patient_360/utils/se_runner.py::run_dq(...)`. SE selects which rules to run
by filtering its rule set on the tuple `(product_id, table_name)`, so `run_dq`
passes the matching identifiers straight from the per-table `dq_rules/{table}.yml`:

- **`product_id`** ← the rules YAML's top-level `product_id` field (e.g.
  `patient-360`) — **never** the physical table name. Passing the table name for
  `product_id` selects zero rules and silently validates nothing (LLD §13
  Decision 16 silent-DQ no-op).
- **`target_table`** ← the resolved `dq_env.<ENV>.table_name` (the **bare**
  physical table name, e.g. `synthea_patients`), with `<ENV>` mapped from the
  runtime `--env` via `_DQ_ENV_MAP` (DEV→DEV, STAGING→QA, PROD→PROD).

Before validation, `run_dq` registers a bare-name temp view for every
`unity.{bronze,silver,gold}.<t>` table so cross-table referential `query_dq` SQL
resolves, then registers the in-flight DataFrame last under its own bare name.

SE **stats** and **error** Delta tables are written to **per-table**, path-based
locations — `warehouse/{env}/_se/<table>/stats` and `warehouse/{env}/_se/<table>/errors`
— so each table's source schema is isolated (a single shared `bronze_se_stats`
path would collide across tables). These stay outside the Unity Catalog catalog
(LLD §13 Decision 12/15).
