...
class TestBuildBronzeTaskgroup:
    ...
    def test_single_yaml_creates_one_task(self, fake_dag, configs_dir, patched_wrapper) -> None:
    ...
    def test_thirteen_yaml_files_creates_thirteen_tasks_sorted(
        ...
        seen_task_ids = [c.kwargs["task_id"] for c in patched_wrapper.call_args_list]
...
class TestConfigsDirResolution:
    ...
    def test_env_var_used_when_kwarg_omitted(
        ...
        assert patched_wrapper.call_count == 1
        assert patched_wrapper.call_args_list[0].kwargs["task_id"] == "ingest_encounters"
...
class TestFilenameHandling:
    def test_yaml_extension_also_picked_up(self, fake_dag, configs_dir, patched_wrapper) -> None:
    ...
    def test_hidden_files_ignored(self, fake_dag, configs_dir, patched_wrapper) -> None:
...
class TestWrapperDelegation:
    def test_pipeline_cfg_and_conn_forwarded(self, fake_dag, configs_dir, patched_wrapper) -> None:
